...
oReport:= TReport():New("FINR15","Retorno Fin. Cobrança","FINR15", {|oReport| ReportPrint(oReport)},"Este relatório irá listar os valores e juros recebidos pela cobrança.")
...
TRCell():New(oSection1,'_VAL000426'		,'', 'PUBLICO' 				, "@E 9,999,999.99" 	,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_JUR000426' 	,'', 'Juros Rec.' 			, "@E 99,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_JRC000426' 	,'', 'Juros Calc.' 			, "@E 99,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_PER000426' 	,'', '% Rec/Calc ' 			, "@E 999,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_MET000426' 	,'', 'CARTEIRA' 			, "@E 99,999,999.99" 	,12 ,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_PRJ000426' 	,'', '% Publico' 			, "@E 99,999.99" 		,12 ,/*lPixel*/,/*{|| code-block de impressao }*/)

TRFunction():New(oSection1:Cell('_VAL000426') ,NIL,"SUM",oBreak,"Total Valor " 	,/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
TRFunction():New(oSection1:Cell('_JUR000426') ,NIL,"SUM",oBreak,"Total Juros " 	,/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
TRFunction():New(oSection1:Cell('_JRC000426') ,NIL,"SUM",oBreak,"% Rec/Calc " 			,/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)

aADD(aCampos,{"_VAL000426"	,"N",12,2})
aADD(aCampos,{"_JUR000426"	,"N",12,2})
aADD(aCampos,{"_JRC000426"	,"N",12,2})
aADD(aCampos,{"_PER000426"	,"N",12,2})
aADD(aCampos,{"_MET000426"	,"N",12,2})
aADD(aCampos,{"_PRJ000426"	,"N",12,2})

TRCell():New(oSection1,'_VAL'  	,'', 'GLOBAL' 			, "@E 99,999,999.99" 	,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_JUR'  	,'', 'Juros Rec.' 		, "@E 999,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_JRC' 	,'', 'Juros Calc.' 		, "@E 99,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_PER' 	,'', '% Rec/Calc ' 		, "@E 99,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_VALT'	,'', 'CARTEIRA' 		, "@E 99,999,999.99" 	,12	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection1,'_PRJ' 	,'', '% Global ' 		, "@E 99,999.99" 		,12	,/*lPixel*/,/*{|| code-block de impressao }*/)

aADD(aCampos,{"_VAL" 	,"N",12,2})
aADD(aCampos,{"_JUR" 	,"N",12,2})
aADD(aCampos,{"_JRC" 	,"N",12,2})
aADD(aCampos,{"_PER" 	,"N",12,2})
aADD(aCampos,{"_VALT" 	,"N",12,2})
aADD(aCampos,{"_PRJ" 	,"N",12,2})
...
TRFunction():New(oSection1:Cell("_JRC"),NIL,"SUM",oBreak,"% Rec/Calc ",/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
...
Local nTxJurPad   	:= SUPERGETMV("MV_TXPER",,0.23) 
...
cQuery := " SELECT E5_FILIAL, E5_DATA, E1_VENCREA, E5_VALOR, E5_VLJUROS, E5_PREFIXO, E5_NUMERO, E5_PARCELA, E5_TIPO, E5_CLIENTE, E5_LOJA, A1_EST, E5_SEQ, A1_SATIV1, E1_VALOR, E1_SALDO "
...
//	aAtend	:= fAtendente( XTT->E5_FILIAL, XTT->E5_CLIENTE, XTT->E5_LOJA, XTT->E1_VENCREA, XTT->E5_DATA)
	...
	*/
	...
	nJurPad := XTT->E1_VALOR * ( 0 + ( (XTT->E5_DATA - XTT->E1_VENCREA) * ( nTxJurPad /100 )))
	
	AADD(aValores, { aAtend[1], aAtend[2], XTT->E5_DATA, XTT->E5_VALOR - XTT->E5_VLJUROS, XTT->E5_VLJUROS, XTT->E5_FILIAL, XTT->E5_DATA - XTT->E1_VENCREA, XTT->E5_PREFIXO, XTT->E1_VENCREA, nJurPad } )
	aAtend	:= {}
...
For x := 1 To Len(aValores)
	...
	cCpVal	:= 'XTS->_VAL000426' //+ AllTrim(aValores[x][1])
	cCpJur	:= 'XTS->_JUR000426' //+ AllTrim(aValores[x][1])
	cCpJurC	:= 'XTS->_JRC000426' //+ AllTrim(aValores[x][1])
	cCpMet	:= 'XTS->_MET000426' //+ AllTrim(aValores[x][1])

	//Valores por dentro pago com até 30 dias
	/*
	If aValores[x][7] <= 30 // se for até 30 dias de recebido. //mv_par03 = 1 // Sintetico

		If  Alltrim(aValores[x][8]) <> '0' .AND. aValores[x][7] < 15 // se for XDD com 15 ou mais dias de recebido.
			Loop
		EndIf
...
//*********************************************
// calculo dos valores totais do setor Público
//*********************************************
...
While !oReport:Cancel() .And. XTT->(!EOF())
	...
	IF !XTS->(dbseek(DtoC(XTT->E5_DATA)))
		...
		XTS->_JRC 		:= nJurPad := XTT->E1_VALOR * ( 0 + ( (XTT->E5_DATA - XTT->E1_VENCREA) * ( nTxJurPad /100 )))
...
While !oReport:Cancel() .And. XTS->(!EOF())
	...
	//While XTM->(!EOF())
		...
		cCpVal	:= 'XTS->_VAL000426'// + AllTrim(XTM->X5_CHAVE)
		cCpJur	:= 'XTS->_JUR000426'// + AllTrim(XTM->X5_CHAVE)
		cCpJurC	:= 'XTS->_JRC000426'// + AllTrim(XTM->X5_CHAVE)
		cCpMet	:= 'XTS->_MET000426'// + AllTrim(XTM->X5_CHAVE)
		
		oSection1:Cell("_VAL000426"):SetValue(&cCpVal)
		oSection1:Cell("_VAL000426"):SetAlign("RIGHT")
		oSection1:Cell("_JUR000426"):SetValue(&cCpJur)
		oSection1:Cell("_JUR000426"):SetAlign("RIGHT")
		oSection1:Cell("_JRC000426"):SetValue(&cCpJurC)
		oSection1:Cell("_JRC000426"):SetAlign("RIGHT")
		/*
		Do Case
		...
		EndCase
		*/
		oSection1:Cell("_PER000426"):SetValue((&cCpJur / &cCpJurC) * 100 )
		oSection1:Cell("_PER000426"):SetAlign("RIGHT")
		oSection1:Cell("_MET000426"):SetValue(&cCpMet)
		oSection1:Cell("_MET000426"):SetAlign("RIGHT")
		oSection1:Cell("_PRJ000426"):SetValue((&cCpVal / &cCpMet) * 100 )
		oSection1:Cell("_PRJ000426"):SetAlign("RIGHT")
		nValTot		+= &cCpVal
		nJurTot		+= &cCpJur
	...
	oSection1:Cell("_JRC"):SetValue(XTS->_JRC)
	oSection1:Cell("_JRC"):SetAlign("RIGHT")
	oSection1:Cell("_PER"):SetValue((XTS->_JUR/XTS->_JRC)* 100)
	oSection1:Cell("_PER"):SetAlign("RIGHT")
	oSection1:Cell("_VALT"):SetValue(XTS->_VALT)
	oSection1:Cell("_VALT"):SetAlign("RIGHT")
	oSection1:Cell("_PRJ"):SetValue((XTS->_VAL/XTS->_VALT)* 100)
	oSection1:Cell("_PRJ"):SetAlign("RIGHT")
...
oReport:Say(oReport:Row(),800,"AREA01 - " + SUPERGETMV("MV_XAREA1",,"AL#BA#CE#PE#RN#SE") )
oReport:SkipLine()
//oReport:Say(oReport:Row(),50,"Comissão - 0,4% a partir de 200% ")
oReport:Say(oReport:Row(),800,"AREA02 - " + SUPERGETMV("MV_XAREA2",,"AC#AM#AP#DF#ES#GO#MA#MG#MS#MT#PA#PB#PI#PR#RJ#RO#RR#RS#SC#SP"))
...
Do Case
	Case nTotalA1 / nMetaA1 >= 1 .and. nTotalA1 / nMetaA1 < 1//1.5
		nFatorA1	:= 0.002
	Case nTotalA1 / nMetaA1 >= 1.5 .and. nTotalA1 / nMetaA1 < 2
		nFatorA1	:= 0.002//0.003
	Case nTotalA1 / nMetaA1 >= 2
		nFatorA1	:= 0.002//0.004
	OtherWise
		nFatorA1	:= 0.002//0
EndCase
	
Do Case
	Case nTotalA2 / nMetaA2 >= 1 .and. nTotalA2 / nMetaA2 < 1//1.5
		nFatorA2	:= 0.002
	Case nTotalA2 / nMetaA2 >= 1.5 .and. nTotalA2 / nMetaA2 < 2
		nFatorA2	:= 0.002//0.003
	Case nTotalA2 / nMetaA2 >= 2
		nFatorA2	:= 0.002//0.004
	OtherWise
		nFatorA2	:= 0.002//0
EndCase
...
For x := 1 To Len(aValores)
	...
	Else
			cCampoD := 'T. XDD'
			If aValores[x][7] < 15 // se for com 15 ou mais dias de recebido.
				Loop
			EndIf
	EndIf
...
IF XTMP->(!EOF())
	...
	Do Case
		Case Alltrim(XTMP->A1_EST) $ cArea2 .AND. XTMP->A1_SATIV1 <> '000009'// AC#AM#AP#DF#ES#GO#MA#MG#MS#MT#PA#PB#PI#PR#RJ#RO#RR#RS#SC#SP
	
			AADD(aRet, "000445") 
			AADD(aRet, "Glaucia") 
	
		Case Alltrim(XTMP->A1_EST) $ cArea1 .AND. XTMP->A1_SATIV1 <> '000009' // AL#BA#CE#PE#RN#SE
...
WHILE d1 <= d2
	...
	cQuery += " AND A1_SATIV1 <> '000009' "
	cQuery += " AND E1_VENCREA BETWEEN '" + DtoS(d1 - 121) + "' AND '" + DtoS(d1-1) + "' " 
	cQuery += " AND E1_TIPO NOT IN ('NCC','RA','JR','DC','CP','MT','XX') "
	//cQuery += " AND E1_PORTADO NOT IN ('COB') " //titulos na global
	...
	While XXTM->(!EOF())
		...
		If nSaldo > 0
			...
			Else
				...
				nTotalX 	:= 0//nTotalX + XXTM->E1_VALOR // AL#BA#CE#RN#SE + XDD
...
//cQuery += " AND A1_SATIV1 = '000009' "
cQuery += " AND E1_VENCREA >= '20100101' " 
...
//							 PUB->E1_CLIENTE,1,StoD(PUB->E1_VENCREA),d1,PUB->E1_LOJA,PUB->E1_FILIAL,,1)
	If nSaldo > 0
		//MsgAlert("PRE" + XXTM->E1_PREFIXO + " NUM " + XXTM->E1_NUM + " PARC " + XXTM->E1_PARCELA + " TIPO " + XXTM->E1_TIPO + " VALOR " + Transform(nSaldo, "@E 999,999,999.99"))
		nTotal 	:= nTotal + nSaldo//PUB->E1_VALOR//nSaldo
...
While GLO->(!EOF())
	...
	dbSelectArea("SE1")
	dbSetOrder(1)
	SE1->(dbSeek(GLO->E1_FILIAL + GLO->E1_PREFIXO + GLO->E1_NUM + GLO->E1_PARCELA + GLO->E1_TIPO))
	//Verifica se foi baixado
	//If !fBaixado(XXTM->E1_FILIAL, XXTM->E1_PREFIXO, XXTM->E1_NUM, XXTM->E1_PARCELA, XXTM->E1_TIPO, XXTM->E1_CLIENTE, XXTM->E1_LOJA, d1)
	...
	nSaldo := SaldoTit(GLO->E1_PREFIXO,GLO->E1_NUM,GLO->E1_PARCELA,GLO->E1_TIPO,GLO->E1_NATUREZ,"R",GLO->E1_CLIENTE,1,,d1,GLO->E1_LOJA,,0,1)
	
//							 GLO->E1_CLIENTE,1,StoD(GLO->E1_VENCREA),d1,GLO->E1_LOJA,GLO->E1_FILIAL,,1)
	If nSaldo > 0
		//MsgAlert("PRE" + XXTM->E1_PREFIXO + " NUM " + XXTM->E1_NUM + " PARC " + XXTM->E1_PARCELA + " TIPO " + XXTM->E1_TIPO + " VALOR " + Transform(nSaldo, "@E 999,999,999.99"))
		nTotal 	:= nTotal + nSaldo//GLO->E1_VALOR//nSaldo
