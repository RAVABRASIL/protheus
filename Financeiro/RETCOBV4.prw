...
oReport:= TReport():New("FINR154","Acompanhamento do Retorno Fin. Cobrança","FINR15", {|oReport| ReportPrint(oReport)},"Este relatório irá listar os valores e juros recebidos pela cobrança.")
...
While XTM->(!EOF())
	...
	TRCell():New(oSection1,'_JRC' + Alltrim(XTM->X5_CHAVE) ,'', 'Juros Calc.'/* + SubStr(XTM->X5_DESCRI)*/ , "@E 9,999,999,999.99" ,16	,/*lPixel*/,/*{|| code-block de impressao }*/)
	TRCell():New(oSection1,'_PRJ' + Alltrim(XTM->X5_CHAVE) ,'', '% Rec/Calc' /* + Alltrim(XTM->X5_DESCRI)*/ , "@E 9,999,999,999.99" ,16	,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	TRCell():New(oSection1,'_PER' + Alltrim(XTM->X5_CHAVE) ,'', '% Recebido ' /* + Alltrim(XTM->X5_DESCRI)*/ , "@E 9,999,999,999.99" ,16	,/*lPixel*/,/*{|| code-block de impressao }*/)
...
//	TRCell():New(oSection1,'_ENT' + Alltrim(XTM->X5_CHAVE) ,'', 'Entrou ' /* + Alltrim(XTM->X5_DESCRI)*/ , "@E 9,999,999,999.99" ,16,/*lPixel*/,/*{|| code-block de impressao }*/)
	...
	TRFunction():New(oSection1:Cell('_JRC' + Alltrim(XTM->X5_CHAVE)),NIL,"SUM",oBreak,"Total Juros " + Alltrim(XTM->X5_DESCRI),/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
	//TRFunction():New(oSection1:Cell('_PRJ' + Alltrim(XTM->X5_CHAVE)),NIL,"AVERAGE",oBreak,"% " + Alltrim(XTM->X5_DESCRI),/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
	//TRFunction():New(oSection1:Cell('_PER' + Alltrim(XTM->X5_CHAVE)),NIL,"AVERAGE",oBreak,"Meta " + Alltrim(XTM->X5_DESCRI),/*Picture*/,/*uFormula*/,.F.,.F.,.F.,oSection1)
	...
	aADD(aCampos,{"_JRC" + Alltrim(XTM->X5_CHAVE)		,"N",16,2})
	aADD(aCampos,{"_PER" + Alltrim(XTM->X5_CHAVE)		,"N",16,2})
	aADD(aCampos,{"_PRJ" + Alltrim(XTM->X5_CHAVE)		,"N",16,2})
...
cQuery += " AND E5_PREFIXO = '0' " //sai o XDD
...
For x := 1 To Len(aValores)
	...
	cCpJurC	:= 'XTS->_JRC'+ AllTrim(aValores[x][1])
	...
	/*	
		IF !XTS->(dbseek(DtoC(aValores[x][3]))) .and. Alltrim(aValores[x][8]) == "0"
		...
		Else
			...
			//Nao grava os que foram recebidos em atraso sem cobrança.
			If !Empty(aValores[x][1]) .and. Alltrim(aValores[x][8]) == "0"
				RecLock("XTS", .F.)
...
While !oReport:Cancel() .And. XTS->(!EOF())
	...
	While XTM->(!EOF())
		...
		cCpJurC	:= 'XTS->_JRC' + AllTrim(XTM->X5_CHAVE)
		...
		oSection1:Cell("_JRC" + Alltrim(XTM->X5_CHAVE)):SetValue(&cCpJurC)
		oSection1:Cell("_JRC" + Alltrim(XTM->X5_CHAVE)):SetAlign("RIGHT")
		oSection1:Cell("_PRJ" + Alltrim(XTM->X5_CHAVE)):SetValue((&cCpJur / &cCpJurC) * 100)
		oSection1:Cell("_PRJ" + Alltrim(XTM->X5_CHAVE)):SetAlign("RIGHT")
		...
		Do Case
			Case	AllTrim(XTM->X5_CHAVE) == '000331'
				oSection1:Cell("_PER" + Alltrim(XTM->X5_CHAVE)):SetValue((&cCpVal / &cCpMet) * 100 )
...
oSection1:Finish()
oReport:Say(oReport:Row(),1110, Transform((nJurTot1/nJurTotC1)*100 , "@E 9,999,999.99") )
oReport:Say(oReport:Row(),2480, Transform((nJurTot2/nJurTotC2)*100 , "@E 9,999,999.99") )
/*
oReport:SkipLine()
oReport:SkipLine()
...
While !oReport:Cancel() .And. XTS->(!EOF())
	...
	oSection1:Cell("_JRC000331"):SetValue(0)
	oSection1:Cell("_JRC000331"):SetAlign("RIGHT")
	oSection1:Cell("_PRJ000331"):SetValue(0)
	oSection1:Cell("_PRJ000331"):SetAlign("RIGHT")

	oSection1:Cell("_JRC000445"):SetValue(0)
	oSection1:Cell("_JRC000445"):SetAlign("RIGHT")
	oSection1:Cell("_PRJ000445"):SetValue(0)
	oSection1:Cell("_PRJ000445"):SetAlign("RIGHT")
