...
/*/
//--------------------------------------------------------------------------
//Programa: FINR002R - Agendamento para representantes
//Objetivo: RELATÓRIO DIÁRIO DE TÍTULOS EM ABERTO DE REPRESENTANTE - LINHA MÉDICO HOSPITALAR
...
If Select( 'SX2' ) == 0
  	///FAZ O ENVIO DE UM ARQUIVO POR REPRESENTANTE
	...
	PREPARE ENVIRONMENT EMPRESA "02" FILIAL "03"
  	...
  	Reset Environment
  	*/
  	///FIM DO ENVIO PARA CADA REPRESENTANTE.
  	/*
  	///FAZ O ENVIO PARA HUMBERTO E FLAVIA VIANA, DE APENAS UM ARQUIVO HTML CONTENDO TODOS OS REPRESENTANTES
  	//ALERT("HUMBERTO .T.")
	...
	PREPARE ENVIRONMENT EMPRESA "02" FILIAL "01"
  	...
  	TitAberto( .T. )	//chama a função do relatório    //TitAberto(lHumberto) -> se lHumberto = .T., envia um arquivo só contendo todos os representantes
  	Reset Environment
	...
	PREPARE ENVIRONMENT EMPRESA "02" FILIAL "03"
  	...
  	Reset Environment
  	*/
  	///FIM DO ENVIO PARA HUMBERTO E FLAVIA VIANA DE APENAS UM ARQUIVO COM TODOS OS REPRESENTANTES
...
*******************************
Static Function TitAberto( lHumberto ) 
*******************************

Local cDesc1         := "Este programa tem como objetivo imprimir relatorio "
Local cDesc2         := "de acordo com os parametros informados pelo usuario."
Local cDesc3         := "Rel. Diario de Titulos em Aberto por Orgao Publico. "
Local cPict          := ""
Local titulo         := "" 
Local Cabec1         := "" 
Local Cabec2		 :=	""
...
Private limite       := 220
Private tamanho      := "G"
Private nomeprog     := "FINR002R" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "FINR002R" // Coloque aqui o nome do arquivo usado para impressao em disco
Private cPerg		 := "FINR002"
Private cString := ""
Private nLin		 := 80
Private cEmpresa:= ""
...
titulo         := "RELATORIO DIARIO DE TITULOS EM ABERTO POR REPRESENTANTE - " + cEmpresa 
...
RunReport(Cabec1,Cabec2,Titulo,nLin,lHumberto)	
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin,lHumberto)
...
//cQuery += "  AND RTRIM(A3_SUPER) = '0316'" + LF      ///TESTE, RETIRAR
...
cQuery += " ,F2_VALMERC, F2_VALBRUT, E1_COMIS1,F2_VALBRUT,E1_VALCOM1, E1_BAIXA, A3_EMAIL  " + LF
...
MemoWrite("C:\Temp\FINR002.sql", cQuery )
...
If Len(acomis) > 0
	...
	If lDentroSiga  
		...
		cDirHTM  := "\Temp\"    
		cArqHTM  := "FINR002R_" + Alltrim(cEmpresa) +".HTM"   
	...
	If lHumberto  
		//se for Schedule criará um arq. por vendedor
		//se estiver dentro do Siga ou Schedule só para Humberto, criará um arquivo contendo todos os vendedores
	...
	Do while nCta <= Len(acomis)
		...
		If !lDentroSiga .and. !lHumberto  
		//se for Schedule, criará um arq. por vendedor, se estiver dentro do Siga, criará um arquivo com todos os vendedores
			
			////CRIA O ARQUIVO DO HTML
			cDirHTM  := "\Temp\"    
			cArqHTM  := "FINR002R_" + Alltrim(cVendedor) + ".HTM"   
			nHandle := fCreate( cDirHTM + cArqHTM, 0 )
		...
		Endif
			...
			Do while nCta <= Len(acomis) .and. Alltrim(acomis[nCta,12]) = Alltrim(cVendedor)
				...
				cNomeModal := U_fNomeModal(acomis[nCta,20])
				//cWeb += '<td width="700" bgcolor="#FFFFC0" align="left"><span class="style3">'+ Alltrim(acomis[nCta,1]) + " / " + Alltrim(acomis[nCta,21]) + '</span></td>'+LF      				//título/parcela
			...
			If !lDentroSiga .and. !lHumberto
				...
				nTotcomis	:= 0
				nTotbonus	:= 0
			Elseif lDentroSiga .and. !lHumberto
			...
			If !lDentroSiga .and. !lHumberto    //FORA DO SIGA
				...
				cMailTo := ""
				//Flávia Rocha - Em 26/07/11 - Regina solicitou que o envio para ela fosse cancelado e incluir Katiane
				//Flávia Rocha - Em 05/09/12 - Katiane solicitou retirar o email dela, pois agora quem desempenhará esta função será Ana Nóbrega
				cMailTo += cMailVend 
				//cMailTo += ";ana.nobrega@ravaembalagens.com.br"    //09/11/12 - Em consulta a Ana Nóbrega, a mesma prefere receber separado por representante
				//cMailTo += ";alcineide@ravaembalagens.com.br"    //08/11/12 - Humberto solicitou retirar o email de Alcineide
				cMailTo += GetNewPar('MV_XFINR2',";comercial@ravaembalagens.com.br")       //25/08/2011 - Solicitado por Janderley incluir o email de Flávio (Licitação)
				//cMailTo += ";humberto.filho@ravaembalagens.com.br" 
				//08/11/12 - Humberto solicitou retirar o email dele do envio individual e inserir no envio de um arquivo apenas com todos os representantes
				

				cCorpo  := titulo + "   - Este arquivo é melhor visualizado no navegador Mozilla Firefox." + CHR(13) + CHR(10)
				...
				If !lEnviou         //faz a tentativa 3 vezes
					//msgbox("email não enviado: " + cVendedor )										
					lEnviou := SendFatr11(cMailTo, cCopia, cAssun, cCorpo, cAnexo )					
					If !lEnviou
						lEnviou := SendFatr11(cMailTo, cCopia, cAssun, cCorpo, cAnexo )					
						If !lEnviou
				...
				Endif
				cWeb := "" 					
			Endif     //se NÃO é DENTRO DO SIGA , e NÃO lHumberto
	...
	If lHumberto
		...
		If Substr(Dtos(dDatabase),7,2) = '01'  //envia para Flavia Viana uma vês ao mês
...
*********************************
User Function fNomeModal(cModal) 
*********************************
