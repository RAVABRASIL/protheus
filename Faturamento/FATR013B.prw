...
//--------------------------------------------------------------------------
//Programa: FATR013B
//Objetivo: Relatório de Acompanhamento por Cliente do Mês  (COLETORES) -  Ano XXXX
...
Private limite       := 220
Private tamanho      := "G"
Private nomeprog     := "FATR013B" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "FATR013B" // Coloque aqui o nome do arquivo usado para impressao em disco
...
titulo         := "Relatorio de Acompanhamento (Vendas de COLETORES) por Cliente (p/ Gerentes)  -  " + cStrAno + ""
...
cQuery += " AND " +LF
cQuery += " RTRIM(SB1.B1_SETOR) = '39' " +LF		//CAIXAS -> SETOR = 39
...
//MemoWrite("C:\Temp\FATR013B.sql", cQuery )
...
Do While nCta <= Len(aDadGeren)
  ...
  If lEhGeren
		...
		/////////////////
		cDirHTM  := "\Temp\"    
		cArqHTM  := "FATR013B" + Alltrim(cGeren) + ".HTM"    //relatório P/ Gerentes
		...
		If nLinhas >= 35
			...
			cWeb += '<tr>        <td>SIGA /FATR013B/v.P10</td>'+ LF
		...
		Do While nCta <= Len(aDadGeren)  .and. Alltrim(cGeren) == Alltrim(aDadGeren[nCta,3]) 	///faça enquanto o gerente atual é igual ao anterior	   	  	   
			...
			If nCta = 1
				...
				cQuery := ""
				cQuery := " SELECT A3_SUPER, SUM(Z7_VALOR) as Z7_VALOR, SUM(Z7_KILO) as Z7QTDE, Z7_MESANO,Z7_TIPO "+LF 
			...
			cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadGeren[nCta,6],"@E 999,999.99")+' </span></td>'+LF        //QTD
			...
			cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadGeren[nCta,8],"@E 999,999.99")+' </span></td>'+LF        //QTD				
			...
			cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadGeren[nCta,10],"@E 999,999.99")+' </span></td>'+LF        //QTD
			...
			//cWeb += '<td width="900" align="right"><span class="style3">'+ Transform(aDadGeren[nCta,12],"@E 999,999.99")+' </span></td>'+LF        //QTD
			...
			//cWeb += '<td width="900" align="right"><span class="style3">'+ Transform(aDadGeren[nCta,14],"@E 999,999.99")+' </span></td>'+LF        //QTD					
			...
			//cWeb += '<td width="900" align="right"><span class="style3">'+ Transform(aDadGeren[nCta,16],"@E 999,999.99")+' </span></td>'+LF        //QTD					
			
			cWeb += '<td width="300" align="right"><span class="style3">'+Transform(aDadGeren[nCta,17],"@E 9,999,999.99") +'</span></td>'+LF  //MEDIA
			nTotMedia += aDadGeren[nCta,17]
		...
		cWeb += '<td width="300" align="right"><span class="style3"><b>' + TRANSFORM( nTotMedia,"@E 9,999,999.99") + '</span></td>'+LF //MEDIA
		cWeb += '</tr>'+LF
...
//MSGINFO("FATR013B - Processo finalizado")
