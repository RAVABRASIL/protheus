...
±±ºPrograma  ³FATR034  º Autor ³ Flávia Rocha        º Data ³  16/11/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Relatorio de Pedidos Liberados no Crédito.                 º±±
±±º          ³                                                            º±±
...
Local cDesc3        := "Pedidos LIBERADOS no CREDITO"
Local cPict         := ""
Local titulo        := "Pedidos LIBERADOS no CRÉDITO"
Local nLin          := 80
...
Private limite      := 90
Private tamanho     := "M"
Private nomeprog    := "FATR034" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo       := 18
...
Private CONTFL      := 01
Private m_pag       := 01
Private wnrel       := "FATR034" // Coloque aqui o nome do arquivo usado para impressao em disco
Private cString     := "SC9" 
...
Private Cab         := "Pedido  Dt.Program  Cliente                                  Liberado em            Valor"
...
//cQuery += " AND (C9_BLCRED = '01' OR C9_BLCRED = '04' OR C9_BLCRED = '09' ) " + LF  //aguardando LIBERAÇÃO CRÉDITO
// NOVA LIBERACAO DE CREDITO
cQuery += " SC9.C9_BLCRED IN( '  ','99') and SC9.C9_BLEST != '10' " + LF
...
//cQuery += " AND SA3.A3_SUPER IN ('0315' ) " + LF //só coordenadores associados a Flavia Viana
cQuery += " GROUP BY C9_FILIAL, C9_PEDIDO ,C5_ENTREG, C5_CONDPAG, A1_COD, A1_NOME, C9_BLCRED " + LF
cQuery += " , C9_DATALIB " + LF
cQuery += " , A3_COD, A3_EMAIL, A3_SUPER " + LF
cQuery += " ORDER BY C9_FILIAL, C5_ENTREG, C9_PEDIDO " + LF
MemoWrite("C:\TEMP\FATR034.SQL", cQuery)
...
While !PEDX->(EOF())
   ...
   if PEDX->C5_CONDPAG = '001'
      ...
      //Aadd( aDados2, {PEDX->C9_PEDIDO,PEDX->C5_ENTREG,PEDX->A1_COD,PEDX->A1_NOME,PEDX->C9_DATALIB, PEDX->TOTAL, PEDX->A3_COD, PEDX->A3_EMAIL, PEDX->A3_SUPER } )
   //else
		//Aadd( aDados1, {PEDX->C9_PEDIDO,PEDX->C5_ENTREG,PEDX->A1_COD,PEDX->A1_NOME,PEDX->C9_DATALIB, PEDX->TOTAL, PEDX->A3_COD, PEDX->A3_EMAIL, PEDX->A3_SUPER } )
   endif   
   //Aadd( aDados1, {PEDX->C9_PEDIDO,PEDX->C5_ENTREG,PEDX->A1_COD,PEDX->A1_NOME,PEDX->C9_DATALIB, PEDX->TOTAL, PEDX->A3_COD, PEDX->A3_EMAIL, PEDX->A3_SUPER, PEDX->C5_CONDPAG } )
   ...
   @nLin,077 PSAY Transform(PEDX->TOTAL, "@E 999,999.99")
	total += PEDX->TOTAL
...
nLin++
nLin++
Cab         := "Pedido  Dt.Program  Cliente                                  Liberado em            "
for nX := 1 to len( aAntec )
   ...
   //@nLin, 64 PSAY iif( aAntec[nX,6] = '09', 'REJEIT. CREDITO', Transform( dDataBase - aAntec[nX,2], "@E 9999" ) )
	@nLin,077 PSAY Transform(aAntec[nX,8] , "@E 999,999.99")
   nLin++
next nX
...
Local nTotHtm2  := 0 
	...
	//cQuery += " AND SA3.A3_SUPER IN ('0315', '0316' , '0320' ) " + LF //só coordenadores associados a Flavia Viana //26/12/13 FAZER PARA TODOS
	...
	MemoWrite("C:\TEMP\FATR034X.SQL", cQuery)
	...
	oProcess:=TWFProcess():New("FATR034","FATR034")
	oProcess:NewTask('Inicio',"\workflow\http\oficial\FATR034.htm")
					...
					If Alltrim(aDados1[x,7]) != Alltrim(aDados1[x - 1,7])
						...
						If lEnvia
							_user := Subs(cUsuario,7,15)
							 ...
							 eEmail += cVendMail
							 eEmail += ";" + cSupMail
 							 If Alltrim(cSuper) = '0315'  //Jacqueline
 							 	eEmail += ";" + "vendas.sp@ravaembalagens.com.br" 
 							 Endif
 							 
 							 If Alltrim(cSuper) = '0342' //Luis Figueira
							 ...
							 subj	:= cVend + " - PEDIDOS LIBERADOS CREDITO - " + cEmpresa
