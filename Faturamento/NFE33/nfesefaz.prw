	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				// Inicio-custonRava
				//Incluido por Eurivan 17/04/09
				if Alltrim(SA1->A1_ENDENT)<>''
 				   cMensCli += "LOCAL ENTREGA: "+Alltrim(SA1->A1_ENDENT)			
				endif
				if Alltrim(SA1->A1_BAIRROE)<>''
 				   cMensCli += " - "+Alltrim(SA1->A1_BAIRROE)			
				endif
				if Alltrim(SA1->A1_MUNE)<>''
 				   cMensCli += " - "+Alltrim(SA1->A1_MUNE)			
				endif
				if Alltrim(SA1->A1_ESTE)<>''
 				   cMensCli += "-"+Alltrim(SA1->A1_ESTE)			
				endif
				if Alltrim(SA1->A1_CEPE)<>''
				...
				Else
					...
					//Inicio-CustonRava
				    //Alterei em  27/04/11 - Eurivan
				    aadd(aDest,IIF(Empty(SA2->A2_CODPAIS),"1058"  ,Alltrim(Str(Val(Posicione("CCH",1,xFilial("CCH")+SA2->A2_CODPAIS,"CCH_CODIGO"))))))
				    aadd(aDest,IIF(Empty(SA2->A2_CODPAIS),"BRASIL",Posicione("CCH",1,xFilial("CCH")+SA2->A2_CODPAIS,"CCH_PAIS")))
				    //Fim-CustonRava
				...
				If !Empty(SF2->F2_TRANSP)
					...
					If !Empty(SF2->F2_VEICUL1)
					    ...
					    //	CustonRava - INICIO 				
					    // QUANDO FOR CLIENTE RETIRA TEM QUE TER PLACA ,UF E NOME DO MOTORISTA 
					    IF ALLTRIM(SF2->F2_TRANSP)='024'
					       dbSelectArea("DA4")
					       DA4->(DbSetOrder(1))
                           If DA4->(MsSeek(xFilial("DA4")+DA3->DA3_MOTORI))
                              aTransp[02]:=DA4->DA4_NOME // NOME DO MOTORISTA QUE VAI FICAR NO LUGAR DO NOME DA TRANSP. CLIENTE RETIRA 
                           ENDIF                    
                        ENDIF
					    //CustonRava - FIM 
...
/*																				
				...
				*/
			// Inicio-CustonRava
			// alterado por antonio customizacao para pegar o volume do pedido
			dbSelectArea("SF2")
			dbSetOrder(1)
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Volumes                                                                 ³
			...
			If SC5->C5_VOLUME1 > 0
				aadd(aEspVol,{ UPPER(C5_ESPECI1),SC5->C5_VOLUME1, SF2->F2_PLIQUI , SF2->F2_PBRUTO})
			    dbSelectArea("SF2")
				dbSetOrder(1)
			Else
			   	dbSelectArea("SF2")
			...
			// Fim-CustonRava
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						// Inicio-CustonRava
						//COLOCADO POR ANTONIO 17/05/2011
						nSaldoNF:=nSaldoNF+(SC6->C6_QTDVEN - SC6->C6_QTDENT)
						cNumPed:=SC5->C5_NUM
						//	
						//Incluído por FR - 06/05/2011
						lMostraPCli := .F.
						///passa como parâmetro o conteúdo do campo C5_OCCLI					
						lMostraPCli := U_FATC017(  SC5->C5_OCCLI  )									
						If lMostraPCli
							cMensCli += "No.ORDEM DE COMPRA CLIENTE: "+Alltrim(SC5->C5_OCCLI)						
						Endif
						//Até aqui - FR - 06/05/2011
						
						//Incluido por Eurivan 28/01/10
						If lEmp
		
							if Alltrim(SC5->C5_NUMEMP)<>''
			 				   cMensCli += "EMPENHO: "+Alltrim(SC5->C5_NUMEMP)						
							Endif
						    ...
						    If !empty( SC5->C5_INFBANC)
						    	
						    	cMensCli += " Inf.Banc. " + AllTrim(SC5->C5_INFBANC)
						    	lEmp:=.F.
				...
				nDescC5 := 0
				nDescC5	:= POSICIONE("SC5",1,xFilial("SC5") + SD2->D2_PEDIDO, 'C5_DESC1')
				//ConvType((aProd[15]+aProd[26])  -> CAMPO DESCONTO , DEPOIS DE FRETE E SEGURO
				
				IF (nDescC5 > 0)  //6DD  - DEU CERTO no UmSiga teste - 18/06/13 - FR
				...
				// Inicio-CustonRava
					...
					cD2Cfop,;
					SB1->B1_UM,;
					(cAliasSD2)->D2_QUANT,;  //qtde do item , e este de baixo, valor total:
					IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR,IIF((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31",(cAliasSD2)->D2_TOTAL,0)),;
					...
					(cAliasSD2)->D2_SEGURO,;
					(0),;  //15
					0,;// O valor unitario sera obtido pela divisao do valor do produto pela quantidade comercial de acordo com o  Manual do Contribuinte 6.00 realizado na tag <vUnCom>(ConvType(aProd[10]/aProd[09],21,8))  //( ( ( (cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT ) + (cAliasSD2)->D2_DESCZFR ) / (cAliasSD2)->D2_QUANT ),;	 //RetPrvUnit(cAliasSD2,0),;  //valor unitário no 6DD o desconto é mascarado através do ,0 -> onde 0 é o param. passado para a função que retorna o preço unitário com ou sem desconto
				...
				ElseIF (nDesconto > 0) .and. nDescC5 = 0  //NF C/ DESCONTO apenas,  não é 6DD!			    
				//bloco do romildo deu certo no Umsiga teste - 19/06/13 - FR
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					(cAliasSD2)->D2_VALFRE,;
					(cAliasSD2)->D2_SEGURO,;
					(nDesconto+nDescIcm),; //valor desconto
					0,;// O valor unitario sera obtido pela divisao do valor do produto pela quantidade comercial de acordo com o  Manual do Contribuinte 6.00 realizado na tag <vUnCom>(ConvType(aProd[10]/aProd[09],21,8))   //RetPrvUnit(cAliasSD2,nDesconto),;      //preço unitário c/ desconto
					IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF
					(cAliasSD2)->D2_LOTECTL,;//Controle de Lote
					(cAliasSD2)->D2_NUMLOTE,;//Numero do Lote
				...
				//fim bloco romildo
					ELSE //sem desconto    deu certo no Umsiga teste 18/06/13 - FR
						aadd(aProd,	{Len(aProd)+1,;
							cCodProd,;
							IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
							cDescProd,;
				...
				ENDIF								
						...
						aadd(aCsosn,{})
						// Inicio-CustonRava
						// chamado 001812  feito em 22/09/2010 (antonio) 	
						cProduto := U_TransGen((cAliasSD2)->D2_COD)
						dbSelectArea("SB5")
						dbSetOrder(1)
						MsSeek(xFilial("SB5")+cProduto)
						
						IF lScLixo
							if ALLTRIM(SB1->B1_SETOR)!='39' // SACO
								if ALLTRIM(SA1->A1_EST) $ "SP/MG/SC/BA"
								   IF  ALLTRIM(SA1->A1_SATIV1) $ '000001/000002/000003/000004/000005/000006'     //SEGMENTO
								       if ALLTRIM(SB1->B1_GRUPO) $ 'A/B/D/E' // GRUPO
								          if ALLTRIM(SB5->B5_CAPACID) $'D/E/F/G/H/I/J/K/L/M/N/O/P' // LITRAGEM
								              cMensCli += ' - Os produtos "sacos de lixo de conteúdo inferior ou igual a 100 litros" (NCM 3923.2) se enquadram no acordo de Substituição Tributária desse Estado. O Estado da Paraíba não é signatário desse acordo. Cabe ao destinatário da presente NF o recolhimento do ICMS substituto.'
								              lScLixo:=.F.
								          endif
								       ELSEIF ALLTRIM(SB1->B1_GRUPO) $ 'C' // GRUPO
								            if ALLTRIM(cProduto) $'CDB154/CGB155/CIB156/CKB157'
								               cMensCli += ' - Os produtos "sacos de lixo de conteúdo inferior ou igual a 100 litros" (NCM 3923.2) se enquadram no acordo de Substituição Tributária desse Estado. O Estado da Paraíba não é signatário desse acordo. Cabe ao destinatário da presente NF o recolhimento do ICMS substituto.'
								               lScLixo:=.F.
								          endif
								       endif
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			EndIf
			// Inicio-CustonRava
			//Incluido por Eurivan em 01/02/11
			if SF1->F1_EST == "EX" 
			   if Empty(SF1->F1_DUPL) //Alterei em 26/04/11 - Eurivan
			      //Essas observações sairão somente nas notas Filhas (ALTERAR NUMERO DA NOTA NO PARAMETRO)
   			      cMensFis := Alltrim(GetMV("MV_MSGIMP1"))
			      cMensFis += Alltrim(GetMV("MV_MSGIMP2"))
			   else		   
			      //Essas observações sairão somente nas NOTA MAE			   
   			      cMensFis := "ACEITAMOS CARTAO - BNDES"
   			      cMensFis += "ICMS REDUZIDO CONFORME(REGIME ESPECIAL DE TRIBUTACAO), "
   			      cMensFis += "CLAUSULA PRIMEIRA DO PARECER NR.2009.01.00.00570, DE"
   			      cMensFis += "18.02.2010, DO RICMS/PB."
   			      cMensFis += "ACEITAMOS CARTAO - BNDES"
                  DbSelectArea("CD5")
				  DbSetOrder(1)
				  // Procura algum registro na CD5 referente a nota que foi complementada
				  If MsSeek(xFilial("CD5")+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA))
				     cMensFis += "DECLARACAO DE IMPORTACAO N.: "+CD5->CD5_NDI+" - "
				     cMensFis += "DATADA DE: "+DtoC(CD5->CD5_DTDI)+" - "+Alltrim(CD5->CD5_LOCDES)+"/"+CD5->CD5_UFDES
			      endif		   		   
			   endif   
			endif
            //Até aqui						
