...
//CUSTON-RAVA - FR - Flávia Rocha - 08/10/13 - Solicitado por Daniela (SAC), imprimir a informação do campo A1_AGENDA
//quando o cliente recebe com agendamento
dbSelectArea("SA1")
dbSetOrder(1)             //DOC    SERIE    FORNECE  LOJA
//If MsSeek(xFilial("SA1")+aNota[5]+aNota[4]+aNota[6]+aNota[7]) .And. SF1->(FieldPos("F1_FIMP"))<>0
If SA1->(DbSeek(xFilial("SA1")+ aNota[6]+aNota[7]) )
	If Alltrim(SA1->A1_AGENDA) = "S"
		cAux := "ENTREGA C/ AGENDAMENTO: SIM " 
	Else
		cAux := "ENTREGA C/ AGENDAMENTO: NAO " 
	Endif

	If Alltrim(SA1->A1_OBSERV) <> ""
		cAux += Alltrim(SA1->A1_OBSERV) 
	Endif
