...
//--------------------------------------------------------------------------
//Programa: FATR012B
//Objetivo: Relatório de Fechamento do Mês  -  Ano XXXX
...
Private limite       := 220
Private tamanho      := "G"
Private nomeprog     := "FATR012B" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "FATR012B" // Coloque aqui o nome do arquivo usado para impressao em disco
...
titulo         := "Relatorio de Fechamento (Vendas de Sacos) do Mes (p/ Gerentes)  -  " + cStrAno + ""
...
//MemoWrite("C:\Temp\fatr012B.sql", cQuery )
...
If Len(aDadG) > 0
	Do While nCta <= Len(aDadG)		
	  ...
	  If lEhGeren
			...
			/////////////////
			cDirHTM  := "\Temp\"    
			cArqHTM  := "FATR012B" + Alltrim(cGeren) + ".HTM"    //relatório P/ Gerentes
			...
			If nLinhas >= 35
				...
				cWeb += '<tr>        <td>SIGA /FATR012B/v.P10</td>'+ LF
			...
			Do While nCta <= Len(aDadG)  .and. Alltrim(cGeren) == Alltrim(aDadG[nCta,3]) 	///faça enquanto o gerente atual é igual ao anterior	   	  	   
				...
				//@nLin,035 PSAY nMetaK		PICTURE "@E 9,999,999.99"     	//KG  meta
				
				////mês 1
				//@nLin,049 PSAY aDadG[nCta,5]		PICTURE "@E 99,999,999.99"  		//R$
				//@nLin,064 PSAY aDadG[nCta,6]		PICTURE "@E 9,999,999.99"     	//KG
				...
				//cWeb += '<td width="900" align="right"><span class="style3">'+ Transform(aDadG[nCta,14],"@E 9,999,999.99")+' </span></td>'+LF        //KG
...
//MSGINFO("FATR012B - Processo finalizado")


dDTPar := LastDay(dDatabase)
PutMV("RV_ULT12B", dDTPar )
