...
Private limite       := 220
Private tamanho      := "G"
Private nomeprog     := "FATR013C" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "FATR013C" // Coloque aqui o nome do arquivo usado para impressao em disco
...
titulo         := "Relatorio de Acompanhamento (Vendas de COLETORES) por Cliente (p/ Coordenadores) - " + cStrAno + ""
...
//MemoWrite("C:\Temp\FATR013C.sql", cQuery )
If Select("SUP") > 0
	DbSelectArea("SUP")
	DbCloseArea()
EndIf
	...
	Do While Alltrim(SUP->C5_CLIENTE) == Alltrim(cCliente) .AND. Alltrim(SUP->C5_LOJACLI) == Alltrim(cLoja)
		Do Case
			Case Month( SUP->C5_EMISSAO ) = nMes1
				nTotVM1 += SUP->VALOR
				nTotPM1 += SUP->QTDE //PESO
			Case Month( SUP->C5_EMISSAO ) = nMes2
				nTotVM2 += SUP->VALOR
				nTotPM2 += SUP->QTDE //PESO
			
			Case Month( SUP->C5_EMISSAO ) = nMes3
				nTotVM3 += SUP->VALOR
				nTotPM3 += SUP->QTDE //PESO
			/*
...
Do while nCta <= Len(aDadSu)
	...
	cDirHTM  := "\Temp\"    
	cArqHTM  := "FATR013C" + Alltrim(cCodSuper) +".HTM"   
	...
	If nLinhas >= 45
		...
		cWeb += '<tr>        <td>SIGA /FATR013C/v.P10</td>'+ LF
		...
		cWeb += '<td width="300" align="center"><span class="style3">QTD</span></td>'+ LF
		cWeb += '<td width="300" align="center"><span class="style3">R$</span></td>'+ LF
		cWeb += '<td width="300" align="center"><span class="style3">QTD</span></td>'+ LF
		cWeb += '<td width="300" align="center"><span class="style3">R$</span></td>'+ LF
		cWeb += '<td width="300" align="center"><span class="style3">QTD</span></td>'+ LF
	...
	Endif		
		...
		Do while nCta <= Len(aDadSu) .and. Alltrim(aDadSu[nCta,6]) == Alltrim(cCodSuper)
		   ...
		   If nCta = 1			   
				...
				//cQuery += " AND RTRIM(A3_SUPER) <> '' "+LF
				cQuery += " AND RTRIM(SZ7.Z7_TIPO) <> 'SC' " + LF      //diferente de SACOS
				cQuery += " AND SA3.A3_FILIAL = '" + xFilial("SA3") + "' AND SA3.D_E_L_E_T_ = ''  "+LF
			...
			*/
			...
			If Alltrim(aDadSu[nCta,22]) = "N"    //se o cliente está Inativo, e o coordenador é Diferente de 0255, apenas "pinta" a linha de cinza
				...
				cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,9],"@E 999,999.99")+' </span></td>'+LF        //QTD					
								
				//coluna mês 2
				cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,10], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">'+ Transform(aDadSu[nCta,11],"@E 999,999.99")+' </span></td>'+LF        //QTD
								
				//coluna mês 3
				cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,12], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">'+ Transform(aDadSu[nCta,13],"@E 999,999.99")+' </span></td>'+LF        //QTD							
				
				//coluna mês 4
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,14], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">'+ Transform(aDadSu[nCta,15],"@E 999,999.99")+' </span></td>'+LF        //QTD					
					
				//coluna mês5
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,16], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">'+ Transform(aDadSu[nCta,17],"@E 999,999.99")+' </span></td>'+LF //QTD						
					    
				//coluna mês 6
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,18], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right" bgcolor="#D0D0D0"><span class="style3">' + Transform(aDadSu[nCta,19],"@E 999,999.99")+' </span></td>'+LF   //QTD
			...
			Else		//se o coordenador é = 0255 mostra apenas os clientes Ativos
				...
				cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,9],"@E 999,999.99")+' </span></td>'+LF        //QTD					
	
				//coluna mês 2
				cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,10], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadSu[nCta,11],"@E 999,999.99")+' </span></td>'+LF        //QTD
								
				//coluna mês 3
				cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,12], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadSu[nCta,13],"@E 999,999.99")+' </span></td>'+LF        //QTD							
						
				//coluna mês 4
				//cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,14], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadSu[nCta,15],"@E 999,999.99")+' </span></td>'+LF        //QTD					
					
				//coluna mês5
				//cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,16], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right"><span class="style3">'+ Transform(aDadSu[nCta,17],"@E 999,999.99")+' </span></td>'+LF //QTD						
												
				//coluna mês 6
				//cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,18], "@E 9,999,999.99")  + '</span></td>'+LF  //R$
				//cWeb += '<td width="300" align="right"><span class="style3">' + Transform(aDadSu[nCta,19],"@E 999,999.99")+' </span></td>'+LF   //QTD
	    ...
	    cWeb := ResumoFim( cCodSuper, cNomeSuper, cWeb, nHandle )
	    cWeb := cWeb + cText
...
cQuery += " AND RTRIM(SB1.B1_SETOR) =  '39' "+LF		//IGUAL A CAIXA
...
While SC6U->( !EOF() )
	...
	If Alltrim(cCliente + cLoja) != Alltrim(cCliAnt + cLojAnt) 
		...
		If SC6U->C5_EMISSAO <= (dDatabase - 91) //(dDatabase - 181)  	  	
			...
			cQuery += " AND B1_SETOR = '39' "+LF		//IGUAL A CAIXA
...
Endif	
	...
	cTexto += '<td width="800" font-family: Arial><div align="center"><span class="style3" style="font-size:14px"><B>CLIENTES QUE NAO COMPRAM HA MAIS DE 3 MESES (VENDAS DE COLETORES):</span></div></B></td>'+ LF
	cTexto += '</tr></table>'+LF
	
	cTexto += '<table width="500" border="0" style="font-size:12px;font-family:Arial">'+LF
	cTexto += '<td width="300"><div align="Left"><span class="style3"><strong>Coordenador: </strong>'+ Alltrim(cCodSuper) + "-" + Alltrim(cNomeSuper) + '</span></td>'+LF					
	cTexto += '</table>' + LF
	...
	For fr:= 1 to Len(aDados)
		...
		cTexto += '<td width="300" align="right"><span class="style3">' + Transform(aDados[fr,8],"@E 999,999.99")+' </span></td>'+LF        //QTD					
