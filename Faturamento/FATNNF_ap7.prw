...
User Function FATNNF()        // incluido pelo assistente de conversao do AP5 IDE em 19/08/02
...
±±³Programa  ³  Nfiscal ³ Autor ³   Silvano Araujo      ³ Data ³ 22/09/99 ³±±
...
If mv_par04 == 2
   ...
   dbSetOrder(1)
   dbSeek(xFilial()+mv_par01+mv_par03+mv_par06+mv_par08,.t.)
...
If mv_par04 == 2
   dbSelectArea("SF2")
   While !eof() .and. SF2->F2_DOC <= mv_par02 .AND. SF2->F2_CLIENTE <= MV_PAR07 .AND.;
          SF2->F2_LOJA <= MV_PAR09 .and. lContinua
      ...
      while !SE1->(eof()) .and. SE1->E1_NUM==xNUM_DUPLIC .and. xDUPLICATAS==.T.
         ...
         */ //comentei pois não imprimimos  mais o boleto direto na rava - Eurivan - 27/08/10
         SE1->(dbSkip())
      EndDo
...
Else
   ...
   While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and.;
      SF1->F1_FORNECE<=mv_par07 .and. SF1->F1_LOJA <= MV_PAR09 .AND. lContinua
