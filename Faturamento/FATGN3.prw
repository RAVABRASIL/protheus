...
@ C(243), C(285) TO C(500),C(520) Dialog _oDlg Title "Informacoes Extras"
  ...
  @ C(092),C(060) Get    cEdit3 object oEdit3 PICTURE "@!" Size 040,100
...
***************
  ...
  IF (dChegd != dTempr2) .AND. (dChegd >= SZZZ->F2_EMISSAO) .AND. (dChegd >= dTempr1)
    if testaDbt( alltrim(SZZZ->F2_DOC) ) .AND. testaCGC()
...
cQuery += "SF2.F2_DOC, SF2.F2_EMISSAO, SA4.A4_NREDUZ, SA1.A1_CONTATO, SZZ.ZZ_PRZENT, SA4.A4_DIATRAB, SA4.A4_CGC "
...
**********
  ...
  IF cDiatrab == alltrim(str(3))
    dData += nPrzent + 1     //O "+1" estava comentado alexandre pediu para descomentar 14/10/09
  Else
    while( x <= nPrzent )
...
aVetor :={ {"E2_PREFIXO" ,'UNI',					Nil},;
			  {"E2_NUM"		 ,SZZZ->F2_DOC,		Nil},;
			  {"E2_PARCELA" ,'1',					Nil},;
			  {"E2_TIPO"	 ,'NDF',					Nil},;                                       
			  {"E2_NATUREZ" ,'21005',				Nil},;
           {"E2_FORNECE" ,POSICIONE("SA2", 3, xFilial("SA2") + SZZZ->A4_CGC,"A2_COD" ),	Nil},;//{"E2_FORNECE" ,'000628',	Nil},;
			  {"E2_LOJA"	 ,'01',					Nil},;
			  {"E2_EMISSAO" ,dDataBase,			Nil},;
			  {"E2_VENCTO"	 ,dDataBase,			Nil},;
			  {"E2_VALOR"	 ,nTotal,				Nil},;
			  {"E2_HIST"	 ,"ATRASO NF: "+alltrim(SZZZ->F2_DOC),  Nil},;
			  {"E2_CODORCA" ,"POSVENDA",			Nil}}
...
***************
  ...
  if !TMPYP->( EoF() )
     msgAlert("Esse registro não pode ser alterado, pois há um débito para o mesmo. ")
     TMPYP->( dbCloseArea() )
     Return .F.
  endIf
  TMPYP->( dbCloseArea() )  
...
Static Function testaCGC( )

***************

Local cCgc := ''

cCgc := POSICIONE("SA2", 3, xFilial("SA2") + SZZZ->A4_CGC,"A2_CGC" )

if empty(cCgc) .OR. empty(SZZZ->A4_CGC)
	msgAlert( "O Cliente está SEM CGC em seu cadastro, favor corrigir!" )
	Return .F.
elseIf alltrim(cCgc) != alltrim(SZZZ->A4_CGC)
	msgAlert( "O CGC cadastrado da transportadora difere do cadastro de fornecedor, favor corrigir!" )
	Return .F.
endIf
