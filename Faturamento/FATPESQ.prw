...
User Function FATPESQ()

Private aRotina := {{"Pesquisar" , "AxPesqui"   , 0, 1},;
                    {"Visualizar", "U_CdaMode2", 0, 2},;
                    {"Incluir"   , "U_CdaMode2", 0, 3},;
                    {"Alterar"   , "U_CdaMode2", 0, 4},;
                    {"Excluir"   , "U_CdaMode2", 0, 5}}

Private cCadastro  := OemToAnsi( "Pesquisa de Mercado" )
Private cAlias1	   := "Z30"	    // Alias de detalhe
Private lSemItens  := .F.		// Permite a nao existencia de itens
Private cChave	   := "Z29_CODIGO"  // Chave que ligara a primeira tabela com a segunda
Private cChave1	   := "Z30_CODPES"  // Chave que ligara a segunda tabela com a primeira
Private aChaves    := {{"Z30_CODPES", "M->Z29_CODIGO"}}
Private cLinhaOk   := "AllwaysTrue()" //Funcao LinhaOk para a GetDados
Private cTudoOk    := "U_TDOK()"//"AllwaysTrue()" //Funcao TudoOk para a GetDados


DbSelectArea("Z29")
DbSetOrder(1)

mBrowse( 06, 01, 22, 75, "Z29" )
...
User Function CdaMode2(cAlias,nReg,nOpc)
...
cSEQ := GetSx8Num( "Z29", "Z29_CODIGO" )
Do While Z00->( DbSeek( xFilial( "Z29" ) + cSEQ ) )
   ConfirmSX8()
   cSEQ := GetSx8Num( "Z29", "Z29_CODIGO" )
EndDo
*/
...
for _t := 1 to len(aCols)
	if empty(aCols[_t][1])
		msgAlert("Código do produto faltando na linha " + alltrim(str(_t)) + "." )
		Return .F.
	elseIf empty(aCols[_t][2]) .and. empty(aCols[_t][3])
		msgAlert("Código do cliente/prospect faltando na linha " + alltrim(str(_t)) + "." )
		Return .F.
	elseIf empty(aCols[_t][4])
		msgAlert("Código do concorrente faltando na linha " + alltrim(str(_t)) + "." )
		Return .F.
	elseIf empty(aCols[_t][5])
		msgAlert("Valor da amostra faltando na linha " + alltrim(str(_t)) + "." )
		Return .F.
	endIf
	/*elseIf empty(aCols[_t][7])
		msgAlert("Quantidade da amostra faltando na linha " + alltrim(str(_t)) + "." )		
		Return .F.
	endIf*/
next
