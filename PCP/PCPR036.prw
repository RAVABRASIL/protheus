...
±±³Programa  ³PCPR036  ³ Autor ³ Gustavo Costa          ³ Data ³14.05.2014³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o   ³Relatorio das horas improdutivas das máquinas.             .³±±
±±³          ³                                                            ³±±
...
oReport:= TReport():New("PCPR36","Disponibilidade de Recursos","PCPR36", {|oReport| ReportPrint(oReport)},"Este relatório irá listar a imdisponibilidade das máquinas.")
...
TRCell():New(oSection1,'RECURS'				,'','Recurso'		,	/*Picture*/				,06				,/*lPixel*/,/*{|| code-block de impressao }*/)
...
TRCell():New(oSection2,'PIPRODP'		,'','% Disp. '			,"@E 999.99"	,06	,/*lPixel*/,/*{|| code-block de impressao }*/)
TRCell():New(oSection2,'TOTALP'			,'','Total Horas Prog.'	,/*Picture*/	,08	,/*lPixel*/,/*{|| code-block de impressao }*/)

oBreak := TRBreak():New(oSection1,oSection1:Cell("RECURS"),"Total das horas no período	:",.F.)
...
oReport:ThinLine()
oReport:SkipLine()
oReport:Say(oReport:Row(),oReport:Col(),"H. Produtivas 	- Qtd. de horas em que a máquina esta disponível para produção.")
oReport:SkipLine()
...
While XH1->(!EOF())
	...
	While d1 <= d2
		...
		cQuery := " SELECT H6_RECURSO, H1_DESCRI, H1_IP, H6_DATAINI, H6_HORAINI, H6_DATAFIN, H6_HORAFIN, H6_TEMPO, "
		cQuery += " CASE WHEN H6_HORAINI BETWEEN '" + cTurno1 + "' AND '13:39:59' THEN '1' ELSE "
		cQuery += " CASE WHEN H6_HORAINI BETWEEN '" + cTurno2 + "' AND '21:59:59' THEN '2' ELSE '3' END END TURNO, "
		cQuery += " H6_MOTIVO, X5_DESCRI "
		cQuery += " FROM  " + RetSqlName("SH6") + " H6 " 
		cQuery += " INNER JOIN " + RetSqlName("SH1") + " H " 
		cQuery += " ON H6_RECURSO = H1_CODIGO "
		cQuery += " INNER JOIN " + RetSqlName("SX5") + " X5 "
		cQuery += " ON X5_TABELA = '44' "
		cQuery += " AND H6_FILIAL = X5_FILIAL "
		cQuery += " AND H6_MOTIVO = X5_CHAVE "
		cQuery += " WHERE H.D_E_L_E_T_ <> '*' "
		cQuery += " AND H6.D_E_L_E_T_ <> '*' "
		cQuery += " AND X5.D_E_L_E_T_ <> '*' "
		cQuery += " AND H6_DATAINI + H6_HORAINI >= '" + DtoS(d1) + cTurno1 + "' " 
		cQuery += " AND H6_DATAINI + H6_HORAINI < '" + DtoS(d1+1) + cTurno1 + "' " 
		cQuery += " AND H6_RECURSO = '" + XH1->H1_CODIGO + "'"
		cQuery += " ORDER BY H6_RECURSO, H6_DATAINI, H6_HORAINI "
		...
		TCSetField( 'TMP', "H6_DATAINI", "D", 8, 0 )
		TCSetField( 'TMP', "H6_DATAFIN", "D", 8, 0 )
		TMP->( DbGoTop() )
		...
		IF TMP->(!Eof())
			...
			If lCabec
				...
				oReport:SkipLine()     
				oSection1:Cell("RECURS"):SetValue(TMP->H6_RECURSO)
				oSection1:Cell("RECURS"):SetAlign("CENTER")
			...
			cRecAnt	:= TMP->H6_RECURSO
	
			While !oReport:Cancel() .And. TMP->(!Eof())
			
				nHora		:= Val(SubStr(TMP->H6_TEMPO,1,3))
				nMin		:= Val(SubStr(TMP->H6_TEMPO,5,2))
				
				nTempo		:= IIF(nHora > 0, nHora*60*60, 0) + nMin * 60
	
				nHImprod	:= nHImprod + nTempo
			...
			oSection2:Cell("DATA"):SetValue(d1)
			oSection2:Cell("DATA"):SetAlign("CENTER")
			oSection2:Cell("IMPROD"):SetValue(fSec4Hora( nHImprod ))
			oSection2:Cell("IMPROD"):SetAlign("RIGHT")
			oSection2:Cell("PIPRODP"):SetValue(Round(((nTotHProg-nHImprod)/nTotHProg)*100,2))
			oSection2:Cell("PIPRODP"):SetAlign("RIGHT")
	...
	If nTotHImprod > 0
		...
		oReport:Say(oReport:Row(),280,fSec4Hora( nTotHImprod ))
		//oReport:Say(oReport:Row(),495,fSec4Hora( nTotHImprod ))
		oReport:Say(oReport:Row(),495,Transform(Round(((nTotHPP-nTotHImprod)/nTotHPP)*100,2),"@E 999.99") )
		oReport:Say(oReport:Row(),670,fSec4Hora( nTotHPP ))
