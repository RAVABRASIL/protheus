...
ฑฑบPrograma  ณNOVO3     บ Autor ณ AP6 IDE            บ Data ณ  21/05/09   บฑฑ
...
Local cDesc3         := "Acompanhamento Grama metro "
Local cPict          := ""
Local titulo         := "PERIODO-Acompanhamento Grama metro "
Local nLin           := 80

Local Cabec1         := "                           Menor Variacao           Maior Variacao   "                      
Local Cabec2         := "Periodo       Valor      Gr/m        Largura      Gr/m      Largura"
Local imprime        := .T.
Local aOrd           := {}
Private lEnd         := .F.
...
Private limite       := 80
Private tamanho      := "P"
Private nomeprog     := "PCPR001" // Coloque aqui o nome do programa para impressao no cabecalho
Private nTipo        := 18
...
Private CONTFL       := 01
Private m_pag        := 01
Private wnrel        := "PCPR001" // Coloque aqui o nome do arquivo usado para impressao em disco
...
wnrel := SetPrint(cString,NomeProg,"PCPR001",@titulo,cDesc1,cDesc2,cDesc3,.T.,aOrd,.T.,Tamanho,,.T.)
...
cQry:="SELECT Z44_MESANO,round((round(SUM(Z44_LARGUR*Z44_ESPESS/10000*Z44_DENSID*100),2)/round(SUM(Z44_PESO/Z44_METROS),2)-1)*100,2) AS TOTAL , "
...
cQry+="GROUP BY Z44_MESANO  "
cQry+="ORDER BY Z44_MESANO "
...
While TMPZ->(!EOF())
   ...
   @nLin++,00 PSAY  TRANSFORM(TMPZ->Z44_MESANO,"@R ##/####")+SPACE(2)+TRANSFORM(TMPZ->TOTAL,"@E 999,999.99" )+;
