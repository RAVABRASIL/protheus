...
User  Function GPE001(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
...
Local cDesc1 := "Emisso de Recibos de Pagamento."
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Private aReturn  := {"Zebrado", 1,"Admistracao", 2, 2, 1, "",1 }	//"Zebrado"###"Administra‡„o"
Private nomeprog :="GPE001"
...
Private cNormal := ALLTRIM(aDriver[2])
...
Private aCodFol:= {}
Private li     := _PROW()
Private Titulo := "EMISSO DE RECIBOS DE PAGAMENTOS"
Private lEnvioOk := .F.
Private lRetCanc	:= .t.
...
IF nTipRel==3
	...
	EndIF*/
    alert("Nao Implementada!!!")
Else
	if nTipRel==1
	   //RptStatus({|lEnd| R030Imp(@lEnd,wnRel,cString,cMesAnoRef,.f.)},Titulo)  // Chamada do Relatorio
        MsAguarde( { || R030Imp(@lEnd,wnRel,cString,cMesAnoRef,.f.) }, "Aguarde. . .", "Contra Cheque . . ." )
    else
       alert("Nao Implementada!!!")
...
/*
IF nTipRel =1 
	SetProc(RecCount())	// Total de elementos da regua
endif
*/
...
oPrt := TMSPrinter():new("Contra Cheque" ) 
oPrt:SetPortrait()
...
While SRA->( !Eof() .And. &cInicio <= cFim )
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
//cMatfun:=SRA->RA_MAT
//While SRA->( !Eof() ) .and. SRA->RA_MAT==cMatfun 	
	IF !( lTerminal )
		/*
		if nTipRel==1
...
Private oFont01, oFont02, oFont03,oFont04,oFont05, oPrint

oFont01 	:= TFont():New('Verdana',12,12,.F.,.T.,5,.T.,5,.F.,.F.)
oFont02 	:= TFont():New('Verdana',10,10,.F.,.F.,5,.T.,5,.F.,.F.)
oFont03 	:= TFont():New('Courier New',08,08,.F.,.F.,5,.T.,5,.F.,.F.)
oFont04 	:= TFont():New('Verdana',08,08,.F.,.T.,5,.T.,5,.F.,.F.)
oFont05 	:= TFont():New('Verdana',06,06,.F.,.T.,5,.T.,5,.F.,.F.)
oFont06     := TFont():New('Verdana',08,08,.F.,.F.,5,.T.,5,.F.,.F.)
//oPrt := TMSPrinter():new("Contra Cheque" ) 
//oPrt:SetPortrait()
...
fRodape3(dDataRef) //Banco de Horas
...
oPrt:Say( 10, 80,"Recibo de Pagamento de Salário" , 		           oFont01, 300, , , 0 )
oPrt:Box(60,   15,220 , 2350, )
oPrt:Say(70, 45,SUBSTR(&cNormal,2,LEN(&cNormal))+DESC_Fil , 		           oFont02, 300, , , 0 )
oPrt:Say(120, 45,DESC_END , 		           oFont02, 300, , , 0 )
oPrt:Say(170, 45,"CNPJ: "+DESC_CGC , 		           oFont02, 300, , , 0 )

If !Empty(Semana) .And. Semana # '99' .And.  Upper(SRA->RA_TIPOPGT) == 'S'
    oPrt:Say(170, 600,"Semana" + Semana + ' (' + cSem_De + "a" + cSem_Ate + ')',oFont02, 300, , , 0 )
Else
    oPrt:Say(170, 600,MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4),oFont02, 300, , , 0 )
EndIf
oPrt:Box(220,   15, 330, 2350, )  
oPrt:Say(230, 45,"Codigo: "+SRA->RA_Mat+"  Nome: "+Left(SRA->RA_NOME,40)+"  CBO: "+fCodCBO(SRA->RA_FILIAL,cCodFunc ,dDataRef)+"  Local: "+SRA->RA_Filial+"  Depto: "+ ALLTRIM(SRA->RA_CC)+"  Seção FL.: "+ transform(ORDEM_REL, "@E 99") , 		           oFont02, 300, , , 0 )
cDet := "Função: "       + cCodFunc						
cDet += cDescFunc     + '  '
cDet += DescCc(SRA->RA_CC,SRA->RA_FILIAL) + ' '
cDet += " CHAPA: " + SRA->RA_CHAPA					
oPrt:Say(280, 45,cDet, 		           oFont02, 300, , , 0 )
oPrt:Box(330,   15, 380, 2350, )    
oPrt:Box(330,   15, 380, 100, )
oPrt:Box(330,   100, 380, 1000, )    
oPrt:Box(330,   1000, 380, 1300, )    
oPrt:Box(330,   1300, 380, 1850, )    
oPrt:Box(330,   1850, 380, 2350, )        
oPrt:Say(340, 25,"Cod.", 		                   oFont04, 300, , , 0 )
oPrt:Say(340, 125,"Descricção", 		           oFont04, 300, , , 0 )
oPrt:Say(340, 1025,"Referência", 		           oFont04, 300, , , 0 )
oPrt:Say(340, 1325,"Vencimentos", 		           oFont04, 300, , , 0 )
oPrt:Say(340, 1875,"descontos", 		           oFont04, 300, , , 0 )

oPrt:Box(380,   15, 1340, 2350, )    
oPrt:Box(380,   15, 1340, 100, )
oPrt:Box(380,   100, 1340, 1000, )    
oPrt:Box(380,   1000, 1340, 1300, )    
oPrt:Box(380,   1300, 1340, 1850, )    
oPrt:Box(380,   1850, 1340, 2350, )        
...
LI ++
@ LI,000 PSAY "| H A B E R E S "		//"| H A B E R E S "
@ LI,046 PSAY "  D E D U C C I O N E S"		//"  D E D U C C I O N E S"
@ LI,090 PSAY "  B A S E S"		//"  B A S E S
@ LI,131 PSAY "|"
...
Static Function fLanca(nConta,nLinha)   // Impressao dos Lancamentos

Local cString := Transform(aLanca[nConta,5],cPict2)
Local nCol := If(aLanca[nConta,1]="P",1575,If(aLanca[nConta,1]="D",2050,27))


/*@ LI,01 PSAY aLanca[nConta,2] // codigo
@ LI,05 PSAY aLanca[nConta,3] // descricao 
If aLanca[nConta,1] # "B"        // So Imprime se nao for base
	@ LI,36 PSAY TRANSFORM(aLanca[nConta,4],"999.99") // referencia
Endif
@ LI,nCol PSAY cString // vencimento ou descontos
Li ++
*/


oPrt:Say(nLinha, 25,   aLanca[nConta,2]                       , 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha, 125,  aLanca[nConta,3]+iif(ncol==27,cString,''), 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha, 1025, TRANSFORM(aLanca[nConta,4],"999.99")   , 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha, nCol, cString                                , 		           oFont03, 300, , , 0 )
...
If cPaisLoc <> "ARG"
	...
	@ LI,044 PSAY "| TOTAL BRUTO     "+SPACE(10)+TRANS(TOTVENC,cPict1)	//"| TOTAL BRUTO     "
	@ LI,088 PSAY "|"+" TOTAL DESCONTOS     "+SPACE(07)+TRANS(TOTDESC,cPict1)	//" TOTAL DESCONTOS     "
	@ LI,131 PSAY "|"
	LI ++
	@ LI,000 PSAY "|"
	...
	@ LI,044 PSAY "| CREDITO:"+SRA->RA_BCDEPSAL+"-"+substr(DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL),1,25)	//"| CREDITO:"
	@ LI,088 PSAY "| LIQUIDO A RECEBER     "+SPACE(05)+TRANS((TOTVENC-TOTDESC),cPict1)			//"| LIQUIDO A RECEBER     "
	@ LI,131 PSAY "|"
	...
	LI ++
	@ LI,000 PSAY "|"
	@ LI,034 PSAY "| CONTA:" + SRA->RA_CTDEPSAL		//"| CONTA:"
	@ LI,088 PSAY "|"
	@ LI,131 PSAY "|"
...
oPrt:Say(1300/*1340*/, 125,DESC_MSG1, 		           oFont01, 300, , , 0 )

oPrt:Box(1340,   15, 1440, 2350, )    
oPrt:Box(1340,   1300, 1440, 1850, )    
oPrt:Line(1390,   1300, 1390, 2350, )        
oPrt:Box(1340,   1850, 1440, 2350, )        
oPrt:Say(1340, 1325,"Total de Vencimento", 		           oFont05, 300, , , 0 )
oPrt:Say(1340, 1575,TRANSFORM(TOTVENC ,cPict1), 		   oFont03, 300, , , 0 )
oPrt:Say(1340, 1875,"Total com Descontos", 		           oFont05, 300, , , 0 )
oPrt:Say(1340, 2050,TRANSFORM(TOTDESC ,cPict1), 		   oFont03, 300, , , 0 )
oPrt:Say(1400, 1575,"Total Líquido --->", 		           oFont05, 300, , , 0 )
oPrt:Say(1400, 2050,TRANSFORM(TOTVENC - TOTDESC ,cPict1),  oFont03, 300, , , 0 )
IF MONTH(dDataRef) = MONTH(SRA->RA_NASC)
oPrt:Say(1340,125 ,"F E L I Z   A N I V E R S A R I O  ! !",oFont01, 300, , , 0 )
ENDIF
IF SRA->RA_BCDEPSAL # SPACE(8)
	Desc_Bco := DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial)	
    oPrt:Say(1390,25 ,"Cred: "+SRA->RA_BCDEPSAL+" - "+DESC_BCO+" Conta: " + SRA->RA_CTDEPSAL,oFont06, 300, , , 0 )
ENDIF
oPrt:Box(1440,15, 1500, 2350, )    
oPrt:Say(1450, 25,"Salário Base", 		           oFont05, 300, , , 0 )
oPrt:Say(1450, 325,"Salário Contr. INSS", 		           oFont05, 300, , , 0 )
oPrt:Say(1450, 625,"Base de Cálc. F.G.T.S.", 		           oFont05, 300, , , 0 )
oPrt:Say(1450, 925,"F.G.T.S. Do Mês", 		           oFont05, 300, , , 0 )
oPrt:Say(1450, 1225,"Base de Cálc. IRRF", 		           oFont05, 300, , , 0 )
oPrt:Say(1450, 1525,"Faixa IRRF", 		           oFont05, 300, , , 0 )
...
oPrt:Say(1470, 25,Transform(nValSal,cPict2),  oFont03, 300, , , 0 )

If Esc = 1  // Bases de Adiantamento
	If cBaseAux = "S" .And. nBaseIr # 0
	    oPrt:Say(1470, 1225,Transform(nBaseIr,cPict1),  oFont03, 300, , , 0 )
	Endif
ElseIf Esc = 2 .Or. Esc = 4  // Bases de Folha e 13o. 2o.Parc.
	If cBaseAux = "S"
		oPrt:Say(1470,325,Transform(nAteLim,cPict1),  oFont03, 300, , , 0 )
		If nBaseFgts # 0 
		    oPrt:Say(1470,625,Transform(nBaseFgts,cPict1),  oFont03, 300, , , 0 )
		Endif
		If nFgts # 0 
		    oPrt:Say(1470,925,Transform(nFgts,cPict2),  oFont03, 300, , , 0 )
		Endif
		If nBaseIr # 	0
		    oPrt:Say(1470,1225,Transform(nBaseIr,cPict1),  oFont03, 300, , , 0 )
		Endif
    	    oPrt:Say(1470,1525,Transform(nBaseIrfE,cPict1),  oFont03, 300, , , 0 )
	Endif
...
oPrt:Box(1500,15, 1600, 2350, )    
oPrt:Say(1550, 25,"Recebi o valor acima em _____/_____/_______ " +space(4)+ Replicate("_",40), 		           oFont02, 300, , , 0 )
oPrt:Say(1640, 0,Replicate("-",2350), 		           oFont02, 300, , , 0 )
...
Static Function fCabec2()   		// Cabecalho do Recibo
Local cCodFunc		:= ""		//-- codigo da Funcao do funcionario
Local cDescFunc		:= ""		//-- Descricao da Funcao do Funcionario
...
oPrt:Say( 1700 /*10*/, 80,"Recibo de Pagamento de Salário" , 		           oFont01, 300, , , 0 )
oPrt:Box(1750/*60*/,   15, 1910/*220*/ , 2350, )
oPrt:Say(1760/*70*/, 45,SUBSTR(&cNormal,2,LEN(&cNormal))+DESC_Fil , 		           oFont02, 300, , , 0 )
oPrt:Say(1810/*120*/, 45,DESC_END , 		           oFont02, 300, , , 0 )
oPrt:Say(1860/*170*/, 45,"CNPJ: "+DESC_CGC , 		           oFont02, 300, , , 0 )

If !Empty(Semana) .And. Semana # '99' .And.  Upper(SRA->RA_TIPOPGT) == 'S'
    oPrt:Say(1860/*170*/, 600,"Semana" + Semana + ' (' + cSem_De + "a" + cSem_Ate + ')',oFont02, 300, , , 0 )
Else
    oPrt:Say(1860/*170*/, 600,MesExtenso(MONTH(dDataRef))+"/"+STR(YEAR(dDataRef),4),oFont02, 300, , , 0 )
EndIf
oPrt:Box(1910/*220*/,   15, 2020/*330*/, 2350, )  
oPrt:Say(1920/*230*/, 45,"Codigo: "+SRA->RA_Mat+"  Nome: "+Left(SRA->RA_NOME,40)+"  CBO: "+fCodCBO(SRA->RA_FILIAL,cCodFunc ,dDataRef)+"  Local: "+SRA->RA_Filial+"  Depto: "+ ALLTRIM(SRA->RA_CC)+"  Seção FL.: "+ transform(ORDEM_REL, "@E 99") , 		           oFont02, 300, , , 0 )
cDet := "Função: "       + cCodFunc						
cDet += cDescFunc     + '  '
cDet += DescCc(SRA->RA_CC,SRA->RA_FILIAL) + ' '
cDet += " CHAPA: " + SRA->RA_CHAPA					
oPrt:Say( 1970/*280*/, 45,cDet, 		           oFont02, 300, , , 0 )
oPrt:Box(2020/*330*/,   15, 2070/*380*/, 2350, )    
oPrt:Box(2020/*330*/,   15, 2070/*380*/, 100, )
oPrt:Box(2020/*330*/,   100, 2070/*380*/, 1000, )    
oPrt:Box(2020/*330*/,   1000, 2070/*380*/, 1300, )    
oPrt:Box(2020/*330*/,   1300, 2070/*380*/, 1850, )    
oPrt:Box(2020/*330*/,   1850, 2070/*380*/, 2350, )        
oPrt:Say(2030/*340*/, 25,"Cod.", 		                   oFont04, 300, , , 0 )
oPrt:Say(2030/*340*/, 125,"Descricção", 		           oFont04, 300, , , 0 )
oPrt:Say(2030/*340*/, 1025,"Referência", 		           oFont04, 300, , , 0 )
oPrt:Say(2030/*340*/, 1325,"Vencimentos", 		           oFont04, 300, , , 0 )
oPrt:Say(2030/*340*/, 1875,"descontos", 		           oFont04, 300, , , 0 )

oPrt:Box(2070/*380*/,   15, 3040/*1340*/, 2350, )    
oPrt:Box(2070/*380*/,   15, 3040/*1340*/, 100, )
oPrt:Box(2070/*380*/,   100,3040/*1340*/, 1000, )    
oPrt:Box(2070/*380*/,   1000, 3040/*1340*/, 1300, )    
oPrt:Box(2070/*380*/,   1300, 3040/*1340*/, 1850, )    
oPrt:Box(2070/*380*/,   1850, 3040/*1340*/, 2350, )        

Return Nil


Static Function fLanca2(nConta,nLinha2)   // Impressao dos Lancamentos

Local cString := Transform(aLanca[nConta,5],cPict2)
Local nCol := If(aLanca[nConta,1]="P",1575,If(aLanca[nConta,1]="D",2050,27))

oPrt:Say(nLinha2, 25,   aLanca[nConta,2]                       , 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha2, 125,  aLanca[nConta,3]+iif(ncol==27,cString,''), 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha2, 1025, TRANSFORM(aLanca[nConta,4],"999.99")   , 		           oFont03, 300, , , 0 )
oPrt:Say(nLinha2, nCol, cString                                , 		           oFont03, 300, , , 0 )

Return Nil


Static Function fRodape2()    // Rodape do Recibo

oPrt:Say(3000/*1340*/, 125,DESC_MSG1, 		           oFont01, 300, , , 0 )

oPrt:Box(3040/*1340*/,   15, 3140/*1440*/, 2350, )    
oPrt:Box(3040/*1340*/,   1300, 3140/*1440*/, 1850, )    
oPrt:Line(3090/*1390*/,   1300, 3090/*1390*/, 2350, )        
oPrt:Box(3040/*1340*/,   1850, 3140/*1440*/, 2350, )        
oPrt:Say(3040/*1340*/, 1325,"Total de Vencimento", 		           oFont05, 300, , , 0 )
oPrt:Say(3040/*1340*/, 1575,TRANSFORM(TOTVENC ,cPict1), 		   oFont03, 300, , , 0 )
oPrt:Say(3040/*1340*/, 1875,"Total com Descontos", 		           oFont05, 300, , , 0 )
oPrt:Say(3040/*1340*/, 2050,TRANSFORM(TOTDESC ,cPict1), 		   oFont03, 300, , , 0 )
oPrt:Say(3100/*1340*/, 1575,"Total Líquido --->", 		           oFont05, 300, , , 0 )
oPrt:Say(3100/*1340*/, 2050,TRANSFORM(TOTVENC - TOTDESC ,cPict1),  oFont03, 300, , , 0 )
IF MONTH(dDataRef) = MONTH(SRA->RA_NASC)
oPrt:Say(3040/*1340*/,125 ,"F E L I Z   A N I V E R S A R I O  ! !",oFont01, 300, , , 0 )
ENDIF
IF SRA->RA_BCDEPSAL # SPACE(8)
	Desc_Bco := DescBco(Sra->Ra_BcDepSal,Sra->Ra_Filial)	
    oPrt:Say(3090/*1390*/,25 ,"Cred: "+SRA->RA_BCDEPSAL+" - "+DESC_BCO+" Conta: " + SRA->RA_CTDEPSAL,oFont06, 300, , , 0 )
ENDIF
oPrt:Box(3140/*1440*/,15, 3200/*1500*/, 2350, )    
oPrt:Say(3150/*1450*/, 25,"Salário Base", 		           oFont05, 300, , , 0 )
oPrt:Say(3150/*1450*/, 325,"Salário Contr. INSS", 		           oFont05, 300, , , 0 )
oPrt:Say(3150/*1450*/, 625,"Base de Cálc. F.G.T.S.", 		           oFont05, 300, , , 0 )
oPrt:Say(3150/*1450*/, 925,"F.G.T.S. Do Mês", 		           oFont05, 300, , , 0 )
oPrt:Say(3150/*1450*/, 1225,"Base de Cálc. IRRF", 		           oFont05, 300, , , 0 )
oPrt:Say(3150/*1450*/, 1525,"Faixa IRRF", 		           oFont05, 300, , , 0 )
...
oPrt:Say(3170/*1470*/, 25,Transform(nValSal,cPict2),  oFont03, 300, , , 0 )

If Esc = 1  // Bases de Adiantamento
	If cBaseAux = "S" .And. nBaseIr # 0
	    oPrt:Say(3170/*1470*/, 1225,Transform(nBaseIr,cPict1),  oFont03, 300, , , 0 )
	Endif
ElseIf Esc = 2 .Or. Esc = 4  // Bases de Folha e 13o. 2o.Parc.
	If cBaseAux = "S"
		oPrt:Say(3170/*1470*/,325,Transform(nAteLim,cPict1),  oFont03, 300, , , 0 )
		If nBaseFgts # 0 
		    oPrt:Say(3170/*1470*/,625,Transform(nBaseFgts,cPict1),  oFont03, 300, , , 0 )
		Endif
		If nFgts # 0 
		    oPrt:Say(3170/*1470*/,925,Transform(nFgts,cPict2),  oFont03, 300, , , 0 )
		Endif
		If nBaseIr # 	0
		    oPrt:Say(3170/*1470*/,1225,Transform(nBaseIr,cPict1),  oFont03, 300, , , 0 )
		Endif
    	    oPrt:Say(3170/*1470*/,1525,Transform(nBaseIrfE,cPict1),  oFont03, 300, , , 0 )
	Endif
ElseIf Esc = 3 // Bases de FGTS e FGTS Depositado da 1¦ Parcela
	If cBaseAux = "S"
		If nBaseFgts # 0
...
oPrt:Box(3200/*1500*/,15, 3300/*1600*/, 2350, )    
oPrt:Say(3250/*1550*/, 25,"Recebi o valor acima em _____/_____/_______ " +space(4)+ Replicate("_",40), 		           oFont02, 300, , , 0 )
...
/*
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿
³Fun‡…o	   ³fRodape3 	³Autor³ Gustavo Costa         ³ Data ³28/06/2013³
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´
³Descri‡…o ³Imprime o extrato do banco de horas do funcionario.         ³
ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
*/

Static Function fRodape3(dData)    // Banco de Horas
...
Local aArea		:= GetArea()
Local nHoras		:= 1
Local dDtIni		:= CtoD("21/" + StrZero(Month(dDataRef - 30),2) +"/"+ StrZero(Year(dDataRef - 30),4))
Local dDtFim		:= CtoD("20/" + StrZero(Month(dDataRef),2) +"/"+ StrZero(Year(dDataRef),4))
...
While !Eof() .And. SPI->PI_FILIAL+SPI->PI_MAT == SRA->RA_FILIAL+SRA->RA_MAT
	...
	*/
	...
	If PosSP9( SPI->PI_PD , SRA->RA_FILIAL, "P9_TIPOCOD") $ "1*3" //SP9->P9_TIPOCOD $ "1*3"
...
oPrt:Say( nLi /*10*/, 80,"Extrato de Banco de Horas" , 		           oFont01, 300, , , 0 )

nLi		:= nLi + 100

cDet := " Saldo Anterior " + Transform(nSaldoAnt,"99999999.99") //"Saldo Anterior "

oPrt:Say(nLi/*1340*/, 80, cDet ,oFont05, 300, , , 0 )

nLi		:= nLi + 60

oPrt:Say( nLi /*10*/, 80,' Data                 Evento                                         Debito     Credito        Saldo   Status' , 		           oFont05, 300, , , 0 )
