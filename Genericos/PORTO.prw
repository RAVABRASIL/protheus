...
Local cDesc3         := "Registro de Controle da Produção e do Estoque - Mod. 3"
Local cPict          := ""
Local titulo       	 := "Registro de Controle da Producao e do Estoque - Mod. 3"
Local nLin         	 := 80
...
Local aArDocs := aArray1  := {}
Local cSeqDc := cCodAnt  := cMes := " "
Local nNotZero := nNotNeg := nEstoque := nTotal  := 0
Local k := 1
...
if (aArray1[1][1] == 'D2' .OR. (aArray1[1][1] == 'D3' .AND. aArray1[1][6] > '500'))
	aArDocs := fwd(aArray1, 1)
	nNotNeg := aArDocs[1][1]
	cSeqDc	:= aArDocs[1][2]
endIf//o primeiro não começa negativo
...
for X:= 1 to len( aArray1 )
  ...
  if ( cMes != substr(aArray1[X][5],5,2) ) .OR. ( cMes == substr(aArray1[X][5],5,2) .AND. (cCodAnt != aArray1[X][3]) )
		...
		if cCodAnt != aArray1[X][3] //zerando o cálculo do estoque do novo produto
			aArRet := marreta( aArray1, X ); k := 1
			nEstoque := saldoIni( alltrim(aArray1[X][3]) )//0//estoque total do produto
			if (aArray1[X][1] == 'D2' .OR. (aArray1[X][1] == 'D3' .AND. aArray1[X][6] > '500'))
				//nNotNeg := fwd(aArray1, X)
				/*if (nEstoque - nNotNeg) < 0
				Criar uma op inicial de crédito no estoque, de modo a não negativar o mesmo em virtude das saídas inicias.
				endIf*/
			endIf
		endIf
  endIf
  ...
  if aArray1[X][1] == 'D3'
  	/*inc*/
		if (aArray1[X + 1][1] == 'D2' .OR. (aArray1[X + 1][1] == 'D3' .AND. aArray1[X + 1][6] > '500')) .AND. !(aArray1[X + 1][2] $ cSeqDc)
			aArDocs := fwd(aArray1, X + 1)
			nNotNeg := aArDocs[1][1]
			cSeqDc	:= aArDocs[1][2]
		endIf
		/*fim inc*/
	    ...
	    @Prow()		 ,014 PSAY transform( aArray1[X][2], "@E 99999999999")//documento
			...
			//nTemp1 :=  aArray1[X][4] + iif(((nEstoque + aArray1[X][4]) - nNotNeg) < 0, -((nEstoque + aArray1[X][4]) - nNotNeg), 0)
			if ((nEstoque + aArray1[X][4]) - nNotNeg) < 0
				nTemp1 	 := aArray1[X][4] + abs( ((nEstoque + aArray1[X][4]) - nNotNeg) )
				nNotZero -= ((nEstoque + aArray1[X][4]) - nNotNeg)
			else
				nTemp1 :=  aArray1[X][4]
			endIf
			@Prow()    ,iif(aArray1[X][6] < '500', 074, 140); //apenas define a coluna
								 PSAY Transform( nTemp1 ,"@E 999,999.99" )					 
			/*iif(aArray1[X][6] < '500', {nTotal 	 += aArray1[X][4],nEstoque += aArray1[X][4]},;
																 {nTotal 	 -= aArray1[X][4],nEstoque -= aArray1[X][4]})*/
			/*inf*/																 
			iif(aArray1[X][6]<'500',{nTotal   += nTemp1, nEstoque += nTemp1,},;
															{nTotal   -= aArray1[X][4],nEstoque -= aArray1[X][4]})
			/*fim inc*/
			//nTestan += 
	    ...
	    @Prow()		 ,014 PSAY transform( aArray1[X][2], "@E 99999999999")//documento
			...
			@Prow()    ,030 PSAY substr(aArray1[X][5],5,2) //mes
			@Prow()		 ,036 PSAY iif(aArray1[X][6] < '500', 'RE', 'RS')
			/*nTemp := (aArray1[X][4] + aArRet[k][2]) + iif(((nEstoque + aArray1[X][4] + aArRet[k][2]) - nNotNeg) < 0 .AND. (aArray1[X + 1][1] == 'D2' .OR. (aArray1[X + 1][1] == 'D3' .AND. aArray1[X + 1][6] > '500')),;
																								   -((nEstoque + aArray1[X][4] + aArRet[k][2]) - nNotNeg), 0)//nTemp é utilizada para marretar o valor do estoque mensal e total*/
			if (((nEstoque + aArray1[X][4] + aArRet[k][2]) - nNotNeg) < 0) .AND. ((aArray1[X + 1][1] == 'D2') .OR. (aArray1[X + 1][1] == 'D3' .AND. aArray1[X + 1][6] > '500'))
			   
				nTemp := (aArray1[X][4] + aArRet[k][2]) + abs ( ((nEstoque + aArray1[X][4] + aArRet[k][2]) - nNotNeg) )
				nNotZero -= ((nEstoque + aArray1[X][4] + aArRet[k][2]) - nNotNeg)
			else
				nTemp := (aArray1[X][4] + aArRet[k][2])
			endIf																								   
			@Prow()    ,iif(aArray1[X][6] < '500', 074, 140); //apenas define a coluna
			PSAY Transform( nTemp,"@E 999,999.99" )//somando com o valor marretado em arret
	...
	elseIf aArray1[X][1] == 'D1'
		...
		@Prow()		 ,053 PSAY aArray1[X][7]					//Conta contabil
		@Prow()		 ,067 PSAY alltrim(aArray1[X][8])//Codigo fiscal
		nTemp3 :=  aArray1[X][4] + iif((((nEstoque + aArray1[X][4]) - nNotNeg) < 0) .AND. (aArray1[X + 1][1] == 'D2' .OR. (aArray1[X + 1][1] == 'D3' .AND. aArray1[X + 1][6] > '500')),;
		                               -((nEstoque + aArray1[X][4]) - nNotNeg), 0)		
...
Static Function marreta( aArrayIn, nInd )
...
***************
	...
	cQuery := "select * from TEMPOR where CODIGO = '"+  aArrayIn[y][3] + "' "
	cQuery := changeQuery( cQuery )
	TCQUERY cQuery NEW ALIAS 'TMP'
	TMP->(  dbGoTop() )
	...
	cCod := aArrayIn[y][3]
	do while cCod == aArrayIn[y][3]
		iif(aArrayIn[y][1] == 'D3',;
				iif(aArrayIn[y][6]<'500',{nTotaliz += aArrayIn[y][4],nTotOP += aArrayIn[y][4],aAdd(aOP,;
				   										   {aArrayIn[y][2],aArrayIn[y][4]})},;
																 {nTotaliz -= aArrayIn[y][4],nTotOP -= aArrayIn[y][4],aAdd(aOP,;
																 {aArrayIn[y][2],aArrayIn[y][4]})}),;
		iif(aArrayIn[y][1] == 'D2',nTotaliz -= aArrayIn[y][4], iif(aArrayIn[y][1] == 'D1',nTotaliz += aArrayIn[y][4], )))
		y++
		if y > len(aArrayIn)
			y--
			cCod := 'XXXXXXXX'
	...
	for i := 1 to len(aOP)
		if nDif > 0 //subtrair		
			...
			aAdd( aReturn, { aOP[i][1],  abs( ((aOP[i][2]*nDif) / nTotOP) ) } )	
		endIf
	Next		
...
Static Function fwd( aArrayIn, nInd )

*************** 

	Local cDocs := cCod := ''
	Local nTotal := 0, y := nInd
	Local aArret := {}
	
	cCod := aArrayIn[y][3]
	do while cCod == aArrayIn[y][3] .AND. (aArrayIn[y][1] == 'D2' .OR. (aArrayIn[y][1] == 'D3' .AND. aArrayIn[y][6] > '500'))
		nTotal += aArrayIn[y][4]
		cDocs  += '/' + aArrayIn[y][2] + ' '
		y++
		if y > len(aArrayIn)
		...
		endIf
	endDo
	if nTotal > 0 .AND. ! empty(cDocs)
		aAdd( aArret, { nTotal, cDocs } )
	endIf
