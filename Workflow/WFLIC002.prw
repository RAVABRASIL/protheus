...
//----------------------------------------------------------------------------------------------
// WFLIC002 - Workflow para envio do relatório Status dos Editais  (Ganha ou Perda) - Diário
// Solicitado pelo Projeto Licitação: Tópico 3.8
...
****************************
User Function WFLIC002()
****************************

If Select( 'SX2' ) == 0
  RPCSetType( 3 )
  PREPARE ENVIRONMENT EMPRESA "02" FILIAL "01" FUNNAME "WFLIC002"    //SACOS
  sleep( 5000 )
  conOut( "Programa WFLIC002 na emp. 02 filial " + xFilial() + " " + Dtoc( Date() ) + ' - ' + Time() )
  Exec()
  //Reset Environment  //DEIXEI NO FINAL DA FUNÇÃO
  
  RPCSetType( 3 )
  PREPARE ENVIRONMENT EMPRESA "02" FILIAL "03" FUNNAME "WFLIC002"    //CAIXAS
  sleep( 5000 )
  conOut( "Programa WFLIC002 na emp. 02 filial " + xFilial() + " " + Dtoc( Date() ) + ' - ' + Time() )
  Exec()
...
Else
  conOut( "Programa WFLIC002 sendo executado pelo MENU ou com erro " + Dtoc( Date() ) + ' - ' + Time() )
  Exec()
EndIf
  conOut( "Finalizando programa WFLIC002 em " + Dtoc( DATE() ) + ' - ' + Time() )
...
cQuery += " and Z17.Z17_STATUS IN ('17', '21') " + LF     //17 - LICITAÇÃO GANHA ou 21 - PERDEMOS
//cQuery += " and Z17.Z17_FILIAL = '" + Alltrim(xFilial("Z17")) + "' " + LF
cQuery += " Order by Z17.Z17_CODIGO " + LF
MemoWrite("C:\TEMP\WFLIC002.SQL" , cQuery )
...
If !TEMP1->(Eof())  
	...
	cAssunto := "Aviso Licitação - GANHAMOS/PERDEMOS: " + Dtoc(dDatabase)
	cMsg     := "Os Seguintes Editais Estão com Status: 17 - LICITAÇÃO GANHA ou 21 - PERDEMOS, Por Gentileza, Analisar e Proceder com Andamento:"
	// Informe o caminho e o arquivo html que será usado.
	cArqHtml := "\Workflow\http\oficial\WFLIC002.html"
		
	// Inicialize a classe de processo:
	oProcess := TWFProcess():New( "WFLIC002", cAssunto )
	...
	Enddo  
		...
		If PswSeek( cSup, .T. )
			aUsers := PSWRET() 						// Retorna vetor com informações do usuário	   
		   	//cNome := Alltrim(aUsers[1][4])		// Nome completo do usuário logado
		   	cMailSup := Alltrim(aUsers[1][14])     // e-mail do usuário logado
		Endif   
		*/
		oProcess:cTo := "" 
