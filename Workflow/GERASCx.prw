...
User Function GERASCX()    //PARA A FILIAL CAIXAS
...
cQuery := "SELECT B1_COD,B1_UM,B1_SEGUM,B1_CONTA,B1_CC,B1_DESC,B1_ESTSEG, B1_ESTSEGX, B1_EMIN, B1_EMINCX, B1_PE, B1_CURVABC, B1_NMAXSC, B1_TIPO," +LF
...
cQuery += "AND B2_QATU <= SB1.B1_ESTSEGX " +LF
cQuery += "AND B1_EMINCX > 0 " +LF	//Inclui Eurivan 01/07/08
cQuery += "AND LEN(B1_COD) <= 7 " +LF
cQuery += "AND B2_FILIAL = '" + xFilial( "SB2" ) + "' " +LF 

//cQuery += " AND RTRIM(B1_COD) in ('ME0101' , 'ME0723' , 'MA0201' , 'ML0124' ) " + LF  //TESTE
//cQuery += " AND RTRIM(B1_COD) in ('MA0201' ) " + LF  //TESTE
...
//TCSetField('SB1X1',"B1_PE","D")


//Produtos Fabricados - Gera OP
cQuery := "SELECT B1_COD,B1_UM,B1_SEGUM,B1_CONTA,B1_CC, B1_DESC, B1_ESTSEG, B1_EMIN , B1_EMINCX " +LF
cQuery += "FROM "+RetSqlName("SB1")+" SB1 " +LF
cQuery += "WHERE B1_ATIVO = 'S' " +LF
cQuery += "AND B1_OPAUTOM = 'S' " +LF
cQuery += "AND B1_EMINCX > 0 " +LF
cQuery += "AND LEN(B1_COD) <= 7 " +LF  
       ...
       //FR - 29/06/13 - SOLICITADO POR ORLEY
		...
		If Alltrim(SB1X1->B1_CURVABC) != "A"  //só pode ter UM, (SC ou PC)
			...
			//If lGera			
			    ...
			    SC1->C1_OBS     := "SC.Pto Pedido: " + Alltrim(Str(SB1X1->B1_ESTSEGX)) + " - ESTQ: " + Alltrim(Str(SB1X1->B2_QATU)) + " Hr: " + Substr(Time(),1,5)
...
//			    	SC1->C1_APROV   := "B"
	    ...
	    Else  //se curva ABC = A
			...
			Else  //If (nQtaSC + nQtoPC) >  0  //só pode ter no máximo um, os produtos com curva ABC = B ou C
				If nQTComprar > (nQTemSC + nQTemPC)
					lGera := .T. //irá gerar se a qtde a comprar do SB1 (B1_EMIN) for maior do que o que está em aberto em SC / PC
				Endif
			Endif
	...
	cMsg   := "WORKFLOW - GERASCX CAIXAS ->"+ Time() + chr(13) + chr(10)
	cMsg   += "Data: " + Dtoc(Date()) + chr(13) + chr(10)
	cMsg   += "Hora: " + Time() + chr(13) + chr(10)
	eEmail := "flavia.rocha@ravaembalagens.com.br"
	subj   := "WORKFLOW - GERASCX CAIXAS ->" + Time()
	U_SendFatr11(eEmail, "", subj, cMsg, "" ) 	
...
If !TMP1->(EOF())
	If Alltrim(cTipo) = "Q"
		//Aadd(aRetorno , { TMP1->QTASSC, TMP1->QTOSPC } )  
		Aadd(aRetorno , { TMP1->QTASSC, TMP1->QTOSPC, TMP1->QTNASC, TMP1->QTNOPC } )  
