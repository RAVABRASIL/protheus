...
±±³Funçäo    ³ MATR240  ³ Autor ³ Eveli Morasco         ³ Data ³ 25/02/93 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descriçäo ³ Saldos em Estoques                                         ³±±
...
Local Tamanho    := 'P'
Local Titulo     := 'Saldos em Estoque' // 'Saldos em Estoque'
Local cDesc1     := "Este programa ira' emitir um resumo dos saldos, em quantidade," // "Este programa ira' emitir um resumo dos saldos, em quantidade,"
Local cDesc2     := 'dos produtos em estoque.' // 'dos produtos em estoque.'
Local cDesc3     := ''
Local cString    := 'SB1'
Local aOrd       := {OemToAnsi(' Por Codigo         '),OemToAnsi(' Por Tipo           '),OemToAnsi(' Por Descricao    '),OemToAnsi(' Por Grupo        ')} // ' Por Codigo         '###' Por Tipo           '###' Por Descricao    '###' Por Grupo        '
Local WnRel      := 'MATR240'
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funçäo    ³ C240IMP  ³ Autor ³ Rodrigo de A. Sartorio³ Data ³ 11.12.95 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descriçäo ³ Chamada do Relatorio                                       ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ MATR240													  ³±±
...
If ( cPaisLoc=="CHI" )
	Tamanho := 'M'
	cPict   := "@E 999,999,999.9999"//"@E 999,999,999.99"
Else
	cPict:= PesqPictQt(If(mv_par15==1,'B2_QATU','B2_QFIM'),17)//PesqPictQt(If(mv_par15==1,'B2_QATU','B2_QFIM'),15)
Endif
...
//³ Monta os Cabecalhos                                          ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
cCabec1 := If(mv_par18<>1,OemToAnsi('CODIGO          TP GRUP DESCRICAO                      UM FL ALM   QUANTIDADE  '),OemToAnsi('CODIGO          TP GRUP DESCRICAO                      UM FL AMZ QTDE.1a.U.M.    UM  QTDE.2a.U.M.')) // 'CODIGO          TP GRUP DESCRICAO                      UM FL ALM   QUANTIDADE  '
cCabec2 := ''
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Processos de Iniciaçäo dos Arquivos Utilizados               ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
#IFDEF TOP
	...
	If mv_par19 == 2 //nao genericos
		cQuery += " and len(SB1.B1_COD) <= 7"
	ElseIf mv_par19 == 3 //genericos
		cQuery += " and len(SB1.B1_COD) > 7"
	EndIf//todos
   ...
   ELSE
	...
	MsAguarde({|| dbUseArea(.T.,'TOPCONN', TCGenQry(,,cQuery), cAliasTOP,.F.,.T.)}, "Selecionando Registros ...") //"Selecionando Registros ..."
	...
	Do While !(cAliasTop)->(Eof())
     aDesc:={}
		...
		If (mv_par14 == 1 .OR. ( mv_par14 <> 1 .and. alltrim(str(nQuant,16,2)) <> "0.00")) .And. ;
		   (mv_par16 <> 1 .Or. ( mv_par16 == 1 .and. If(mv_par14 <> 1,alltrim(str(nQuant,16,2)) < "0.00",alltrim(str(nQuant,16,2)) <= "0.00")))
			...
			If mv_par17 == 1 // Desc. Cientifica
				...
				//aDesc := quebra(alltrim(SB5->B5_CEME), 30)
			    aDesc :=wordWrap(alltrim(SB5->B5_CEME), 30)
			Endif
            
             If(Empty(aDesc)) 
             //aDesc := quebra(alltrim((cAliasTop)->DESCRI), 30)
			 aDesc:= wordWrap(alltrim((cAliasTop)->DESCRI), 30)
			 EndIF
		   ...
		   //	@ Li, 24 + nCOL1 pSay If(Empty(cDesc),Left((cAliasTop)->DESCRI, 30),cDesc)
			...
			If mv_par18==1
			   ...
			   @ Li, 84 + nCOL1 pSay Transform( ConvUm((cAliasTop)->COD,nQuant,0,2), cPict)
			   @ Li, 101/*99*/ + nCOL1 pSay SubStr(If(SubStr((cAliasTop)->SITU,1,1)$'2', "Bloqueada ", "Disponivel"), 1, 1)
			Else
			   @ Li, 81/*79*/ + nCOL1 pSay SubStr(If(SubStr((cAliasTop)->SITU,1,1)$'2', "Bloqueada ", "Disponivel"), 1, 1)
			Endif 
			 
            for y := 1 to len(aDesc)
			@ Li++, 24 + nCOL1 pSay aDesc[y][1]
			Next
			
			Li++
			nTOTAL1 += nQuant
			nTOTAL2 += ConvUm((cAliasTop)->COD,nQuant,0,2)
		...
		If !(nTotQuebra==0) .And. 	If(aReturn[8]==4,	!(cGrupoAnt==GRUPO) .Or. ((cAliasTop)->(EOF()) .And.Empty(cGrupoAnt)),If(aReturn[8]==2,!(cTipoAnt==TIPO),.F.)) .Or. ;
			...
			@ Li, 40 + nCOL1 pSay If(Empty(cMens),SubStr('Total do ',1,5),'Total do ' + cMens) //'Total do '
			@ Li, 63 + nCOL1 pSay Transform(nTotQuebra, cPict)
			If mv_par18 == 1
			   @ Li, 81 + nCOL1 pSay (cAliasTop)->SEGUM
			   @ Li, 84 + nCOL1 pSay Transform(ConvUm(cCodAnt,nTotQuebra,0,2), cPict)
			Endif
			Li += 2
			nTotQuebra := 0
		Endif
		...
		.or. ( ( lVEIC) .and. (!((cCodite + cCodAnt) == (cAliasTop)->(CODITE + cod))))
			If nQtdProd > 1 .And. Alltrim(Str(aReturn[8])) $ "1|3" //-- So' totaliza Produto se houver mais de 1
				If (!(nTotProd==0).Or.!(nTotProdBl==0))
					@ Li, 24 + nCOL1 pSay "(" + SubStr("Disponivel",1,1)+ ") = " + OemToAnsi("Qtde. ") + OemToAnsi("Disponivel") + Replicate('.',14)   //"Qtde. "###"Disponivel  "
					@ Li, 63 + nCOL1 pSay Transform((nTotProd-nTotProdBl), cPict)
					If mv_par18 == 1
					   @ Li, 84 + nCOL1 pSay Transform(ConvUm(cCodAnt,(nTotProd-nTotProdBl),0,2), cPict)
					Endif
					Li++
					@ Li, 24 + nCOL1 pSay "(" + SubStr("Bloqueada ",1,1)+ ") = " + OemToAnsi("Qtde. ") + OemToAnsi("Bloqueada ") + Replicate('.',14)   //"Qtde. "###"Bloqueada "
					@ Li, 63 + nCOL1 pSay Transform(nTotProdBl, cPict)
					If mv_par18 == 1
					   @ Li, 84 + nCOL1 pSay Transform(ConvUm(cCodAnt,(nTotProdBl),0,2), cPict)
					Endif
					Li++
					If ! lVEIC
						@ Li, 24 + nCOL1 pSay OemToAnsi('Total do Produto') + Space(1) + AllTrim(Left(cCodAnt,15)) // 'Total do Produto'
					ELSE
						@ Li, 24+ nCOL1 pSay OemToAnsi('Total do Produto') + Space(1) + (cCodite  + " " + cCodAnt) // 'Total do Produto'
					ENDIF
					@ Li, 63 + nCOL1 pSay Transform(nTotProd, cPict)
					If mv_par18 == 1
					   @ Li, 84 + nCOL1 pSay Transform(ConvUm(cCodAnt,nTotProd,0,2), cPict)
					Endif
					Li += 2
				EndIf
			EndIf
	...
	If mv_par18==1
		 @ ++Li, 45 PSAY "TOTAIS ---------->"
	   @ Li, 63 + nCOL1 pSay Transform( nTOTAL1, cPict )
	   @ Li, 84 + nCOL1 pSay Transform( nTOTAL2, cPict )
  Endif
#ELSE
	...
	IndRegua('SB2',cNomArqB2,cIndB2,,cFiltroB2,'Selecionando Registros...') //'Selecionando Registros...'
	nIndB2 := RetIndex('SB2')
	...
	IndRegua('SB1',cNomArqB1,cIndB1,,cFiltroB1,'Selecionando Registros...') //'Selecionando Registros...'
	nIndB1 := RetIndex('SB1')
	...
	//-- 1º Looping no Arquivo Principal (SB1)
	Do While !SB1->(Eof()) .and. lContinua
		...
		//-- 2º Looping no Arquivo Principal (SB1)
		Do While !SB1->(Eof()) .And. (cQuebra1 == If(aReturn[8]==1.Or.aReturn[8]==3,.T.,&(cCampo))) .And. lContinua
			...
			For nX := 1 to Len(aFiliais)
				...
				//-- 1º Looping no Arquivo Secundário (SB2)
				Do While lContinua .And. !SB2->(Eof()) .And. B2_COD == SB1->B1_COD
					...
					//-- 2º Looping no Arquivo Secundário (SB2)
					Do While lContinua .And. !SB2->(Eof()) .And. &(cCond2)
						...
						If !Empty(SB2->B2_FILIAL)
							...
							If SM0->(DBSeek(cEmpAnt+SB2->B2_FILIAL, .F.))
								//-- Atualiza a Variavel utilizada pela funçÆo xFilial()
						   ...
						   //	@ Li, 24 + nCOL1 pSay Left(If(lIsCient, cDesc,	fContSB1(SB2->B2_FILIAL, SB2->B2_COD, 'B1_DESC')),30)
							...
							If mv_par18 == 1
                               @ Li, 81 + nCOL1 pSay fContSB1(SB2->B2_FILIAL, SB2->B2_COD, 'B1_SEGUM')
   							   @ Li, 84 + nCOL1 pSay Transform( If(mv_par15==1,ConvUm(B2_COD,B2_QATU,0,2),If(mv_par15==2,ConvUm(B2_COD,B2_QFIM,0,2),ConvUm(B2_COD,aSalProd[1],0,2))), cPict)
						 	   @ Li, 101/*99*/ + nCOL1 pSay SubStr(If(SubStr(B2_STATUS,1,1)$"2","Bloqueada ","Disponivel"),1,1)
						 	Else
					 	       @ Li, 81/*79*/ + nCOL1 pSay SubStr(If(SubStr(B2_STATUS,1,1)$"2","Bloqueada ","Disponivel"),1,1)
						    Endif
							 
							//aDesc:=quebra(alltrim(If(lIsCient, cDesc,	fContSB1(SB2->B2_FILIAL, SB2->B2_COD, 'B1_DESC')),30))
						      aDesc:=wordWrap(alltrim(If(lIsCient, cDesc,	fContSB1(SB2->B2_FILIAL, SB2->B2_COD, 'B1_DESC')),30))
						    for z:=1 to len(aDesc)
						    @ Li++, 24 + nCOL1 pSay aDesc[z][1] 
						    Next
						...
						If mv_par01 == 1
							If SB2->B2_COD # cCodAnt .And. ;
								...
								If nQtdProd > 1
									...
									If mv_par18 == 1
									   @ Li, 84 + nCOL1 pSay Transform(ConvUm(SB1->B1_COD,(nSoma-nQtdBlq),0,2), cPict)
									Endif
									Li++
									@ Li, 24 + nCOL1 pSay "(" + SubStr("Bloqueada ",1,1)+ ") = " + OemToAnsi("Qtde. ") + OemToAnsi("Bloqueada ") + Replicate('.',14)   //"Qtde. "###"Bloqueada "
									@ Li, 63 + nCOL1 pSay Transform(nQtdBlq, cPict)
									If mv_par18 == 1
									   @ Li, 84 + nCOL1 pSay Transform(ConvUm(SB1->B1_COD,nQtdBlq,0,2), cPict)
                  Endif
									Li++
				 					If ! lVEIC
										@ Li, 24+ nCOL1 pSay OemToAnsi('Total do Produto') + Space(1) + AllTrim(Left(cCodAnt,15)) // 'Total do Produto'
									Else
										@ Li, 24+ nCOL1 pSay OemToAnsi('Total do Produto') + Space(1) + SB1->B1_CODITE  + " " + cCodAnt // 'Total do Produto'
									Endif
									@ Li, 63 + nCOL1 pSay Transform(nSoma, cPict)
									If mv_par18 == 1
									   @ Li, 84 + nCOL1 pSay Transform(ConvUm(SB1->B1_COD,nSoma,0,2), cPict)
									Endif
									Li += 2
									nQtdBlq := 0
								EndIf
						...
						ElseIf !(nSoma == 0 .And. mv_par14 == 2) .Or. (nSoma >= 0  .And. mv_par16 == 1)
							...
							//aDesc:=quebra(alltrim(If(lIsCient, cDesc,	fContSB1(cFilOld, cCodAnt, 'B1_DESC')),30))
							 aDesc:=wordWrap(alltrim(If(lIsCient, cDesc,	fContSB1(cFilOld, cCodAnt, 'B1_DESC')),30))
							for w:=1 to len (aDesc)
							@ Li++, 24 + nCOL1 pSay aDesc[w][1]
							Next
		...
		If (aReturn[8] == 2 .Or. aReturn[8] == 4) .And. ;
			nTotSoma # 0
			@ Li, 40 + nCOL1 pSay 'Total do ' + cMens //'Total do '
			@ Li, 63 + nCOL1 pSay Transform(nTotSoma, cPict)
			If mv_par18 == 1
			   @ Li, 84 + nCOL1 pSay Transform(ConvUm(SB1->B1_COD,nTotSoma,0,2), cPict)
			Endif
			Li += 2
			nTotSoma := 0
		EndIf
...
//-- Retorna a PosiçÆo Correta do SM0
SM0->(dbGoto(nRegM0))
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funçäo    ³ fContSB1 ³ Autor ³ Fernando Joly Siquini ³ Data ³ 13.10.98 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descriçäo ³ Procura produto em SB1 e retorna o conteudo do campo       ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Sintaxe   ³ fContSB1( cChave, cCampo)                                  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Parâmetros³ cFil   = Filial de procura                                 ³±±
±±³Parâmetros³ cCod   = Codido de procura                                 ³±±
±±³          ³ cCampo = Campo cujo conteúdo se deseja retornar            ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ Generico                                                   ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
