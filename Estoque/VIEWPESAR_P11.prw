...
SetPrvt( "nQTDLIB, cUSULIM, nLIMITE, cSEQ, cNOMUSUA,oChkFOP,lFinal,nPorig, lCapa" )
Private cPrdt   := ""
Private cRegRed := ""
Private cMAQ	:= ""
Private nLastPesagem	:= 0
Private nLimQUANT	:= GETNEWPAR("MV_XLIMQTD",1) //Valida a quantidade de fardos digitados na pesagem
...
oSayQFD:cCaption := "Qtd. &Fardos Permitido 1 por vez:"
oSayQFD:nLeft := 654
oSayQFD:nTop := 445
oSayQFD:nWidth := 66
oSayQFD:nHeight := 18
oSayQFD:lShowHint := .F.
oSayQFD:lReadOnly := .F. // Colocar apenas um fado por vez.
oSayQFD:Align := 0
...
oGetQFD:nHeight := 21
oGetQFD:lShowHint := .F.
oGetQFD:lReadOnly := .F. // Colocar apenas um fado por vez.
oGetQFD:Align := 0
...
//  vai vai funcionar depois que iventariar os produtos 
IF nValendo
	if PRIMEIROCX(SC2->C2_NUM)
...
If SECONDS() - nLastPesagem < 10
	MsgAlert("TROQUE O PRODUTO DA BALANÇA !!!!")
	Return nil
EndIf

nLastPesagem := SECONDS()
//ObjectMethod( oPESO, "SetText( Trans( nPESO, '@E 999.99' ) )" )
ObjectMethod( oSayPKG02, "SetText( Trans( nPESO, '@E 999.99' ) )" )
if( lCapa, pesaCapa(), Grava() )
Return NIL
...
//Chamado 1170 de lindenberg, 28/05/09
if substr(SC2->C2_PRODUTO,1,2) $ "CL CM CN CO CP"//Caixas perfurocortante
	nPct := 0.94 //invertido em 29/09/09 a pedido de Lindenberg
else//Sacos e embalagens
	nPct := Getmv( "MV_PESOTOL" ) //0.965 //invertido em 29/09/09 a pedido de Lindenberg
endIf
...
Do While .T.
	...
	If ! lIncomp
       ...
       nPESOMSEC := 0                                                                
		...
		Do While ! SG1->( Eof() ) .and. SG1->G1_COD == SB1->B1_COD
           ...
           ElseIf alltrim(SB1->B1_TIPO) $ "MS/AC"
      ...
      If nPESO < Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * nPct, 3 )//Mínimo     //Antes ( 1 - ( nTOLERA / 100 ) ), 3 ) * nPct
		   Alert("O produto pesado esta MUITO ABAIXO do peso limite.  So podera ser liberado pela Diretoria.")
		   aUSUARI2 := U_senha2( "01", 1, "Margem de peso menor que o limite" )
		   If ! aUSUARI2[ 1 ]
			  latua := .F.
		   	  Exit
		   else
			   If SuperGetMV("RV_INSPSP",,.T.)		   
			   	  cCodISP := InspSP(cOP,cMaq,nQUANT) //Verifica codigo da inspecao de sobrepeso
			   	  if Empty(cCodISP)
				     latua := .F.
	   		   	     Exit
	   		      else
	                 lISP:=.T. 
	                 //AtuISP(cCodISP,nQUANT)   		   	   		      	   		   	
	   		      endif			   	   			   				   
			   ENDIF
		   EndIf
      EndIf

      If nPESO > Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) , 3 ) * ((1 - nPct) + 1) //Máximo
			Alert("O produto pesado esta MUITO ACIMA do peso limite.  So podera ser liberado pela Diretoria.")
		   aUSUARI2 := U_senha2( "01", 1, "Margem de peso maior que o limite" )
		   If ! aUSUARI2[ 1 ]
			  latua := .F.
		   	  Exit
		   else
      ...
      EndIf
       ...
       //Se Quant. Liberada menor Quant. Fardos *ou* Se o peso for maior que o máximo
       If nQTDLIB < nQUANT .or. nPESO > Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * ((1 - nPct) + 1), 3) // Máximo //antes ( 1 - ( nTOLERA / 100 ) ), 3 ) * 1.1 
           //Se o Peso for menor que o mínimo *ou* Se o peso for maior que o máximo
           If nPESO < Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * nPct, 3 ) .or. nPESO > Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * ((1 - nPct) + 1), 3 )
           
           	If ! MsgBox( "Produto " + AllTrim( SB1->B1_COD ) + " - " + SB1->B1_DESC + Chr( 13 ) + Chr( 13 ) + "Com peso (" + AllTrim( Trans( nPESO, '@E 999.99' ) ) + ") fora da faixa permitida (" + AllTrim( Trans( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * nPct, ;
              "@E 999.999" ) ) + " -- " + Alltrim( Trans( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * ((1 - nPct) + 1), "@E 999.999" ) ) + ;
              "). Sacos capa (" + AllTrim( Trans( nPESOCAPA, "@E 999.999" ) ) + "). Material secundario (" + AllTrim( Trans( nPESOMSEC, "@E 999.999" ) ) + "), Confirma?", "Escolha", "YESNO" )
				...
				aUSUARIO := U_senha2( "01", 3 )
				   ...
				   If SuperGetMV("RV_INSPSP",,.T.)   		   	    
	   		   	      IF !lISP // SO PEDI A INSPECAO AQUI SE NAO TIVER ENTRADO EM "MUITO ACIMA" OU "MUITO ABAIXO"
		   		   	       cCodISP := InspSP(cOP,cMaq,nQUANT) //Verifica codigo da inspecao de sobrepeso
				   	       if Empty(cCodISP)
					          latua := .F.
		   		   	          Exit
				...
				If nQTDLIB > 0   // Cria registro de quantidade de fardos liberados
					...
					Z01->( DbCommit() )
				EndIf
			EndIf//Se o Peso for menor que o mínimo *ou* Se o peso for maior que o máximo
			
		Else//Se Quant. Liberada menor Quant. Fardos *ou* Se o peso for maior que o máximo
		...
		EndIf//Se Quant. Liberada menor Quant. Fardos *ou* Se o peso for maior que o máximo
		...
		For nCONT := 1 To nQUANT
       	...
       	cSEQ := ProxSeq()
			...
			AllTrim( SB1->B1_COD ), "A" + AllTrim( Left( SB1->B1_DESC, 39 ) ), Left( Z00->Z00_OP, 6 ) + Z00->Z00_SEQ } )
	...
	Else //Incompleto
       ...
       nPESOMSEC := 0
		...
		If nPESO < Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * nPct, 3 ) //Minimo
		...
		ElseIf nPESO > Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) , 3 ) * ((1 - nPct) + 1) //Máximo
			...
			Elseif  nPESO < Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * nPct, 3 );
			.or. nPESO > Round( ( nPROPORCAO + nPESOCAPA + nPESOMSEC + nDIFPESO ) * ((1 - nPct) + 1), 3 )
			...
			//Aadd( aIMP, { "B" + "Rava Embalagens", "A" + AllTrim( Left( SB1->B1_DESC, 39 ) ), Left( cOP, 6 ) + cSEQ } )
			Aadd( aIMP, { "B" + "Rava Embalagens", "B" + iif(SC2->C2_OPLIC == "S","Amostra: ","Incompleto: ") +;
			AllTrim( SB1->B1_COD ), "A" + AllTrim( Left( SB1->B1_DESC, 39 ) ), Left( Z00->Z00_OP, 6 ) + Z00->Z00_SEQ } )
...
// DEPOIS QUE IMPRIME A ETIQUETA DO PA CONSOME OS ITENS DA ESTRUTURA DO PA
IF nValendo
	if PRIMEIROCX(SC2->C2_NUM)
...
//For i := 1 To Len( aIMP )
	...
	MSCBSAY(08,14,aIMP[ 1, 2 ],"N","C","14")	// Horizontal
...
//	MSCBSAY(39,16,cCapac,"N","0","55",.T.)	// Horizontal
	...
	MSCBSAYBAR(10,18,aIMP[ 1, 4 ],"N","C",10,.F.,.T.,.F.,,2,1,.F.) 
...
/*20/04/09 Pesagem de sacos-capa comprados.*/
SC7->( dbSetOrder(14) )
/*20/04/09 Pesagem de sacos-capa comprados.*/
If SC2->C2_NUM + SC2->C2_ITEM + SC2->C2_SEQUEN <> cOP
	if msgYesNo("Tratam-se de sacos-capa comprados?") .and. SC7->( dbSeek( xFilial( "SC7" ) + substr(cOP,1,6), .T. ) )//20/04/09 Pesagem de sacos-capa comprados.
		SC7->( dbSetOrder(4) )
		cPrdt := produto()
		if SC7->( dbSeek( xFilial( "SC7" ) + cPrdt + substr(cOP,1,6), .T. ) )
			lCapa := lPedido := lRet := .T.//20/04/09 Pesagem de sacos-capa comprados.
			cOP := Left( cOP, 6 )
		else
			msgAlert("Por favor, escolha um produto válido!")
			lRET := .F.
		endIf
	else//20/04/09 Pesagem de sacos-capa comprados.
		if msgYesNo("Trata-se de uma devolução de sacos-capa?")
			cPrdt := produto()
			if ExistCpo("SB1",cPrdt) .and. !empty(cPrdt)
				lCapa := lPedido := lRet := .T.//20/04/09 Pesagem de sacos-capa comprados.
			else
				lRET := .F.
			endIf
		else
			MsgAlert( OemToAnsi( "OP ou Pedido de compas nao cadastrado(a)!" ) )
			lRET := .F.
		endIf
	endIf
Else
...
***************
Static Function AtuPeso(nQUANT)
***************
	//Valida a quantidade de fardos digitados na pesagem
	If nQUANT > nLimQUANT
	
		MsgAlert("Quantidade informada superior ao limite de fardos!")
		Return .F.
...
if nOpt == 2
	...
	If  aUSUARIO[ 1 ]	
		...
		aEncerra := { 	{ "D3_OP"			, padr(alltrim(cOP),14)	, NIL },;
						  { "D3_TM" 		, "110"					, NIL },;
		...
		Begin Transaction 
			...
			lMsErroAuto := .F.
			MsgRun("Encerramento da OP. Aguarde...","MATA250 - Encerramento de OP",{|| MSExecAuto({|x,y| MATA250(x,y)},aEncerra, 6) })
			If lMsErroAuto
				DisarmTransaction()
			   MostraErro()
			Else
			 	MsgAlert("OP Encerrada!!!!")
			EndIf
...
elseIf nPTeor <= 0
	...
	Return
elseIf nPeso > (nPTeor * nTolera)      //5% A MAIS
	msgAlert("Peso MUITO ACIMA do limite!")
	if ! U_senha2("01",1)[1]
		Return
	endIf
	cLibera := "Alexandre"
elseIf nPeso < (nPTeor * ((1 - nTolera) + 1))     //5% A MENOS
...
Static Function PI_CONSOME(cCod,cOPPA)
...
*************************************
Static Function InspSP(cOP,cMaq,nPct)
*************************************
...
Ù±±ÀÄÄÄ   ÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ*/
Private cInsp := Space(6)

/*ÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±± Declaração de Variaveis Private dos Objetos                             ±±
Ù±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ*/
SetPrvt("oDlgI","oGrpI","oSayI","oGetInsp","oSBtnI1","oSBtnI2")

/*ÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±± Definicao do Dialog e todos os seus componentes.                        ±±
Ù±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ*/
oDlgI            := MSDialog():New( 278,374,385,780,"Informe a Inspeção Sobrepeso",,,.F.,,,,,,.T.,,,.F. )
oGrpI            := TGroup():New( 004,004,040,146,"",oDlgI,CLR_BLACK,CLR_WHITE,.T.,.F. )
oSayI            := TSay():New( 013,011,{||"Código Inspeção:"},oGrpI,,,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,051,008)
oGetInsp         := TGet():New( 011,069,,oGrpI,060,008,'',,CLR_BLACK,CLR_WHITE,,,,.T.,"",,,.F.,.F.,,.F.,.F.,"Z93","cInsp",,)
oGetInsp:bSetGet := {|u| If(PCount()>0,cInsp:=u,cInsp)}
oGetInsp:bValid  := {||ExistCpo("Z93")}

oSBtnI1           := SButton():New( 007,157,1,{||lOk := .T.,FOK(nPct) },oDlgI,,"", )
oSBtnI2           := SButton():New( 023,157,2,{||lOk := .F.,cInsp := Space(6),oDlgI:End()},oDlgI,,"", )

oDlgI:Activate(,,,.T.)


IF EMPTY(cInsp) .AND. lOk 
   
   InspSP(cOP,cMaq,nPct)
...
************************************
Static Function AtuISP(cCodISP,nQtd)
************************************

DbSelectArea("Z93")
DbSetOrder(1)
DbSeek(xFilial("Z93")+cCodISP)
RecLock("Z93",.F.)
Z93->Z93_QTDLIB += nQtd
Z93->(MsUnLock())
...
*********************
Static Function fOk(nPct)
*********************

Local nQtIpSP := SuperGetMV("RV_QTIPSP",,15) 

DbSelectArea("Z93")
DbSetOrder(1)
DbSeek(xFilial("Z93")+cInsp)

if substr(cOP,1,6) <> alltrim(Z93->Z93_OP)

	Alert("A OP( "+substr(cOP,1,6)+" ) da pesagem é diferente da OP( "+alltrim(Z93->Z93_OP)+" ) da Inspeção.")
	cInsp := Space(6)
    oDlgI:End()
    Return 
...
if alltrim(cMaq) <> alltrim(Z93->Z93_MAQUIN)

	Alert("A MAQUINA( "+alltrim(cMaq)+" ) da pesagem é diferente da MAQUINA( "+alltrim(Z93->Z93_MAQUIN)+" ) da Inspeção.")
	cInsp := Space(6)
    oDlgI:End()
    Return 
...
if (Z93->Z93_QTDLIB+nPct) >= nQtIpSP
	
	if Z93->Z93_QTDLIB = 999
		Alert("A inspeção selecionada não é válida, pois NÃO É JUSTIFICADA.")
	else
		Alert("A inspeção selecionada não é válida, pois NÃO POSSUI SALDO.")
	endif
	cInsp := Space(6)
    oDlgI:End()
    Return 
