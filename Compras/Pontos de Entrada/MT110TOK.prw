...
Local nPosDtEn    	:= aScan(aHeader,{|x| AllTrim(x[2]) == 'C1_DATPRF'})
...
Next
/*  //FR - 15/04/14 - chamado #78 - AIRTON SOLICITOU QUE NÃO PEDISSE MAIS A SENHA DE ORLEY/MARCELO NA DEFINIÇÃO DA COMPRA URGENTE
If lUrgente
...
// ENVIAR EMAIL PARA QUEM ABRIU A SC 
PswOrder(1)               			
If PswSeek( __CUSERID , .T. ) 			
   	aUsuSC   := PSWRET() 	  		
	cNomeSo  :=ALLTRIM(aUsuSC[1][4]) 
	cEmailSo := ';'+ALLTRIM(aUsuSC[1][14]) 
Endif
...
dbSelectArea("SM0")
if INCLUI
   cConteudo +=" Empresa - " + SM0->M0_FILIAL +" Incluida por : "+cNomeSo+"<br>"
   cTitulo	:= " Incluida Solicitação de Compra - " + cA110Num
ELSEif ALTERA
   cConteudo +=" Empresa - " + SM0->M0_FILIAL +" Alterada por : "+cNomeSo+"<br>"
   cTitulo	:= " Alterada Solicitação de Compra - " + cA110Num
ELSE
...
cConteudo +="            <td align='center' colspan='8' >"
...
// NOVO 
cConteudo +="            <td align='center' width='100' bgcolor='#CCFFCC'>"
cConteudo +="                <strong>Dt.Prev.Entrega </strong></td>"
//
cConteudo +="        </tr>"
...
// novo 
	cConteudo +="            <td align='rigth' width='100'>"
	cConteudo += DtoC(aCols[x][nPosDtEn]) + "</td>"
//
	cConteudo +="        </tr>"
...
//Totalizador
cConteudo +="<tr>"
cConteudo +="    <td height='23' colspan='4' bgcolor='#CCFFCC'><strong> TOTAL </strong></td>"
...
If SubStr(aCols[1][nPosCC],1,1) $ "7" //Produção
	cMailTo 	:= GetNewPar('MV_XMASC1',"giancarlo.sousa@ravaembalagens.com.br")
Else
	cMailTo 	:= GetNewPar('MV_XMASC2',"giancarlo.sousa@ravaembalagens.com.br")
EndIf

cMailTo += cEmailSo // MANDA TAMBEM PARA QUEM ABRIU A SC 
cMailTo += ';compras@ravaembalagens.com.br' // MANDA TAMBEM PARA QUEM ABRIU A SC 

lEnviado := U_SendFatr11( cMailTo, cCopia, cTitulo, cConteudo, cAnexo )

If lEnviado
	MsgAlert("Solicitação enviada por E-mail para você e seu diretor !! ")
Else
